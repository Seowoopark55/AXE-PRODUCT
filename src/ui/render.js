...
function setupGuidePreview(state){
  const demo=state.setupDemo||{}; const step=Math.max(0,Math.min(5,Number(demo.step||0))); const connected=Boolean(demo.connected);
  const steps=[['WELCOME','시작'],['DISCORD','서버 연결'],['ROLES','역할'],['MODULES','기능'],['CHANNELS','채널'],['READY','완료']];
  ...
  if(step===0){ body=`<div class="setup-demo-hero"><span class="setup-demo-kicker">AXE PRODUCT SETUP GUIDE</span><h2>처음 설정도, 순서대로 하면 어렵지 않습니다.</h2><p>필요한 것만 하나씩 안내하고 완료된 단계는 자동으로 확인합니다. 지금 체험하는 내용은 실제 회사 설정에 저장되지 않습니다.</p><div class="setup-demo-facts"><span><b>약 2–3분</b> 예상 소요시간</span><span><b>5단계</b> 핵심 설정만 진행</span><span><b>0건</b> 실제 데이터 변경</span></div></div>`; nextLabel='체험 시작'; }
  ...
  if(step===3){ body=`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 03</span><h2>회사에서 사용할 기능만 선택합니다.</h2><p>켜지 않은 기능은 다음 단계에서도 묻지 않습니다.</p><div class="setup-demo-module-grid">${moduleCard('fund','공금 관리','납부 · 검수 · 원장')}${moduleCard('ammo','총알 관리','3시 · 10시 주문/제작')}${moduleCard('outlaw','무법지대 전적','등록 · 랭킹 · 기록')}${moduleCard('cooking','요리 주문','주문 · 변경 · 영업')}${moduleCard('assets','자산 · 계좌','배정 · 반납 · 계좌')}</div></div>`; }
  if(step===4){ const rows=[]; if(modules.fund)rows.push(channelSelect('공금-현황','공금 현황 채널',demo.channels?.fund)); if(modules.ammo){rows.push(channelSelect('3시-총알','3시 총알 채널',demo.channels?.ammo3));rows.push(channelSelect('10시-총알','10시 총알 채널',demo.channels?.ammo10));} if(modules.outlaw)rows.push(channelSelect('전적-등록','무법 전적 등록 채널',demo.channels?.outlaw)); if(modules.cooking)rows.push(channelSelect('요리-주문','요리 주문 채널',demo.channels?.cooking)); body=`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 04</span><h2>필요한 Discord 채널만 연결합니다.</h2><p>앞 단계에서 켠 기능에 필요한 채널만 보여줍니다.</p><div class="setup-demo-channel-list">${rows.length?rows.join(''):`<div class="setup-demo-empty"><strong>채널 설정이 필요한 기능이 없습니다.</strong><span>바로 다음 단계로 진행할 수 있습니다.</span></div>`}</div><div class="setup-demo-tip"><strong>가이드 원칙</strong><span>사용하지 않는 기능의 설정은 처음부터 보여주지 않습니다.</span></div></div>`; }
  if(step===5){ body=`<div class="setup-demo-complete"><div class="setup-demo-complete-mark">✓</div><span class="setup-demo-kicker">SETUP COMPLETE</span><h2>운영 준비가 완료되었습니다.</h2><p>실제 초기설정에서는 여기까지 완료되면 운영 콘솔로 자연스럽게 이동합니다.</p><div class="setup-demo-next-task"><span>NEXT</span><div><strong>멤버를 등록하세요.</strong><small>Discord에서 대상 우클릭 → 앱 → AXE 멤버 등록</small></div></div><div class="setup-demo-safe"><b>체험 모드</b><span>지금 선택한 역할·기능·채널은 실제 회사 설정에 저장되지 않았습니다.</span></div></div>`; nextLabel='체험 종료'; }
  const primary=step===1&&!connected?`<button type="button" class="setup-demo-primary" data-action="setup-demo-connect">Discord 연결 체험</button>`:step===5?`<button type="button" class="setup-demo-primary" data-action="setup-demo-finish">체험 종료</button>`:`<button type="button" class="setup-demo-primary" data-action="setup-demo-next">${nextLabel}</button>`;
  return `<div class="setup-demo-backdrop"><section class="setup-demo-shell" role="dialog" aria-modal="true" aria-label="초기설정 가이드 체험"><aside class="setup-demo-rail"><div class="setup-demo-brand"><strong>AXE PRODUCT</strong><small>GUIDED SETUP · PREVIEW</small></div><div class="setup-demo-preview-badge"><i></i><span>테스트 설정</span><small>실제 데이터 변경 없음</small></div><nav>${rail}</nav><button type="button" class="setup-demo-exit" data-action="close-modal">체험 닫기</button></aside><main class="setup-demo-main"><header><div><span>초기설정 가이드 체험</span><strong>${step+1} / ${steps.length}</strong></div><button type="button" data-action="close-modal" aria-label="닫기">×</button></header><div class="setup-demo-content">${body}</div><footer>${step>0&&step<5?`<button type="button" class="setup-demo-back" data-action="setup-demo-back">이전</button>`:'<span></span>'}<div><button type="button" class="setup-demo-restart" data-action="setup-demo-restart">처음부터</button>${primary}</div></footer></main></section></div>`;
}
