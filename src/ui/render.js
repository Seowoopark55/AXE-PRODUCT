...
function renderMembers(state){
  ...
  const memberRows=rows.length?rows.map(m=>{
    ...
    return `<tr>
      ...
      <td data-label="역할"><strong>${esc(ROLE_KO[m.role]||m.role)}</strong></td>
      <td data-label="입사일"><strong>${esc(hire)}</strong></td>
