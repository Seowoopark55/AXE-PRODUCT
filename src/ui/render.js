...
function renderFundLedger(state) {
  ...
  return `<section class="axe-fund-history"><header class="axe-fund-history-head"><div><h2>공금내역</h2><p>필요한 정보만 빠르게 확인하고, 상세 작업은 행에서 바로 처리합니다.</p></div><div class="axe-fund-history-head-actions"><select class="axe-fund-history-select axe-fund-history-select--month" data-fund-ledger-month>${monthOptions(state.fundMonth,12)}</select><button class="axe-fund-primary axe-fund-primary--ledger" data-action="open-ledger">수입·지출 등록</button></div></header>
    ...
    <div class="axe-fund-history-columns"><span>날짜</span><span>이름</span><span>내역</span><span>금액</span><span>증빙</span><span>관리</span></div>
    <div class="axe-fund-history-list">${rows.length?rows.map(r=>renderLedgerRow(r,state)).join(''):'<div class="axe-fund-history-empty">조건에 맞는 공금내역이 없습니다.</div>'}</div></section></section>`;
}
function renderLedgerRow(r,state){ const amount=Number(r.amount||0); const source=r.request_id?'승인':'직접'; const isWeeklyPayment=r.entry_type==='payment'; const title=isWeeklyPayment?'주간공금':(r.category||'기타'); const sub=[isWeeklyPayment?'공금납부':r.ledger_type,r.direction,r.memo].filter(Boolean).join(' · '); const current=(state.memberships||[]).find(m=>m.id===r.membership_id); const who=current?.display_name||r.member_display_name||'—'; const key=dateKey(r.ledger_date); const [y,m,d]=key.split('-'); return `<article class="axe-fund-history-row"><div class="axe-fund-history-date-cell"><strong>${m}.${d}</strong><span>${y}</span></div><div class="axe-fund-history-who"><strong>${esc(who)}</strong><span>${esc(r.account||'—')}</span></div><div class="axe-fund-history-entry"><div class="axe-fund-history-title"><strong>${esc(title)}</strong><span class="axe-fund-history-kind axe-fund-history-kind--${r.request_id?'approval':'manual'}">${source}</span></div><span>${esc(sub||'—')}</span></div><div class="axe-fund-history-money ${amount<0?'is-expense':'is-income'}">${signedMoney(amount)}</div><div class="axe-fund-history-evidence">${r.evidence_path?`<button class="axe-fund-history-action is-evidence" data-action="open-evidence" data-evidence-path="${esc(r.evidence_path)}">증빙</button>`:'<span class="axe-fund-history-no-evidence">—</span>'}</div><div class="axe-fund-history-manage">${r.can_edit?`<button class="axe-fund-history-action" data-action="edit-ledger" data-entry-id="${esc(r.id)}">수정</button>`:'<span class="axe-fund-history-no-evidence">—</span>'}</div></article>`; }
...
function renderAssetBoard(state){ let rows=(state.assetsSnapshot?.assets||[]).slice(); const q=(state.assetQuery||'').toLowerCase(); if(state.assetCategory)rows=rows.filter(a=>a.asset_category===state.assetCategory); if(state.assetStatus)rows=rows.filter(a=>(a.membership_id?'사용중':'미배정')===state.assetStatus); if(q)rows=rows.filter(a=>`${a.asset_name||''} ${a.owner_name||''} ${a.asset_category||''}`.toLowerCase().includes(q)); const cats=[...new Set((state.assetsSnapshot?.assets||[]).map(a=>a.asset_category).filter(Boolean))]; return `<section class="ops-mgmt-board"><div class="ops-mgmt-toolbar"><div class="ops-mgmt-filters"><label class="ops-mgmt-search">${icon('search')}<input data-asset-query value="${esc(state.assetQuery||'')}" placeholder="자산명 · 보유자 검색"></label><select class="ops-mgmt-select" data-asset-category><option value="">분류 전체</option>${cats.map(c=>`<option ${state.assetCategory===c?'selected':''}>${esc(c)}</option>`).join('')}</select><select class="ops-mgmt-select" data-asset-status><option value="">상태 전체</option><option ${state.assetStatus==='사용중'?'selected':''}>사용중</option><option ${state.assetStatus==='미배정'?'selected':''}>미배정</option></select></div></div><div class="ops-mgmt-meta"><span><strong>${rows.length}</strong>개 표시</span><span>퇴사자의 자산은 자동으로 미배정되어 다시 배정할 수 있습니다.</span></div><div class="ops-mgmt-column-head ops-asset-column-head"><span>보유자</span><span>자산</span><span>분류</span><span>상태</span><span>관리</span></div><div class="ops-mgmt-list">${rows.length?rows.map(a=>`<article class="ops-mgmt-row ops-asset-row"><div class="ops-asset-holder"><strong>${esc(a.owner_name||'미배정')}</strong><span>현재 보유자</span></div><div class="ops-asset-main"><strong>${esc(a.asset_name)}</strong><span>자산명</span></div><div class="ops-asset-category"><strong>${esc(a.asset_category||'기타')}</strong><span>분류</span></div><div class="ops-asset-state"><span class="ops-mgmt-badge ${a.membership_id?'is-green':'is-amber'}">${a.membership_id?'사용중':'미배정'}</span></div><button class="ops-mgmt-action ${a.membership_id?'':'is-assign'}" data-action="edit-asset" data-asset-id="${esc(a.id)}">${a.membership_id?'상세':'배정'}</button></article>`).join(''):empty('조건에 맞는 자산이 없습니다.')}</div></section>`; }
