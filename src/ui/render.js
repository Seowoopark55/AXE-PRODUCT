...
function settingsSaveLabel(state,resetBusy){
...
function renderOnboardingProgress(state){
  ...
  return `<section class="ops-onboarding-progress"><div class="ops-onboarding-progress__copy"><strong>${phase==='ready'?'초기 설정 완료':'초기 설정'}</strong><span>${esc(guide)}</span></div><div class="ops-onboarding-steps">${labels.map(([key,no,label],i)=>`<span class="${i<activeIndex||phase==='ready'?'is-done':i===activeIndex?'is-current':''}"><b>${i<activeIndex||phase==='ready'?'✓':no}</b>${label}</span>`).join('')}</div></section>`;
}
function roleName(id,roles){return roles.find(r=>r.role_id===id)?.role_name||'';}
function renderBasicSettings(state,roles){ const cfg=state.discordCompanyConfig||{}; const connected=state.discordConnection?.status==='connected'; const resetBusy=['reset_requested','resetting'].includes(String(state.onboardingStatus?.status||'')); const discordAction=resetBusy?'':connected?'open-discord-reconnect':'connect-discord'; const discordLabel=resetBusy?'정리 중':connected?'다시 설정':'연결'; return `<form id="settings-active-form" data-form="settings-basic" class="ops-settings-board ops-settings-basic"><div class="ops-settings-board-head"><div><h2>기본 정보</h2><p>회사와 Discord에서 공통으로 사용하는 핵심 값입니다.</p></div></div>${settingRow('회사 이름','서비스와 Discord 안내에 표시',`<input name="brand_name" value="${esc(companyDisplayName(state))}" ${resetBusy?'disabled':''}>`,'company')}${settingRow('관리자 역할','공금 · 멤버 · 자산 · 설정 관리',`<select name="admin_role_id" ${resetBusy||!connected?'disabled':''}><option value="">선택 안 함</option>${roles.map(r=>`<option value="${esc(r.role_id)}" ${cfg.admin_role_id===r.role_id?'selected':''}>${esc(r.role_name)}</option>`).join('')}</select>`,'role')}${settingRow('일반 멤버 역할','회사 기능을 사용하는 일반 멤버',`<select name="member_role_id" ${resetBusy||!connected?'disabled':''}><option value="">선택 안 함</option>${roles.map(r=>`<option value="${esc(r.role_id)}" ${cfg.member_role_id===r.role_id?'selected':''}>${esc(r.role_name)}</option>`).join('')}</select>`,'role')}${settingRow('Discord 서버',connected?'현재 연결된 서버 · 필요하면 안전하게 다시 설정할 수 있습니다.':'BOT을 사용할 Discord 서버를 연결합니다.',`<div class="ops-settings-discord ${connected?'is-connected':'is-disconnected'} ${resetBusy?'is-busy':''}"><i></i><strong>${esc(state.discordConnection?.guild_name||'미연결')}</strong><button type="button" ${discordAction?`data-action="${discordAction}"`: 'disabled'}>${discordLabel}</button></div>`,'discord')}</form>`; }
