...
function renderCookingMenuSettings(state){
  ...
  const guide=`<form data-form="cooking-guide" class="ops-cooking-guide-card"><div class="ops-cooking-guide-head"><div><strong>Discord 주문 안내</strong><span>${esc(guideHint)}</span></div><button type="submit" class="ops-compact-save" ${guideDisabled?'disabled':''}>${icon('save')}<span>안내 저장</span></button></div><div class="ops-cooking-guide-grid"><label><span>정기 주문일</span><input name="schedule_text" maxlength="200" value="${esc(config?.schedule_text||'')}" placeholder="예: 매주 화요일 / 목요일 / 토요일" ${guideDisabled?'disabled':''}></label><label><span>1 SET 기준</span><input name="set_guide" maxlength="500" value="${esc(config?.set_guide||'')}" placeholder="예: 고기 10 · 감자 3 · 양파 3 · 당근 3" ${guideDisabled?'disabled':''}></label><label class="is-wide"><span>추가 안내</span><textarea name="extra_guide" maxlength="1000" rows="2" placeholder="예: 재료 준비나 주문 관련 안내" ${guideDisabled?'disabled':''}>${esc(config?.extra_guide||'')}</textarea></label></div></form>`;
  ...
  return `${guide}<section class="ops-settings-board ops-cooking-menu-board"><div class="ops-settings-board-head ops-cooking-menu-head"><div><h2>요리 주문 메뉴</h2><p>회사별 메뉴 · 설명 · 가격 · 노출 순서를 관리합니다.</p></div><div class="ops-cooking-menu-tools"><span class="ops-cooking-menu-count"><strong>${active}</strong> / ${rows.length} 사용 중</span><button type="button" class="ops-cooking-add" data-action="open-cooking-menu" ${!admin?'disabled':''}>${icon('plus')}<span>메뉴 추가</span></button></div></div><div class="ops-cooking-menu-hint">활성 메뉴만 Discord 주문창에 표시됩니다. 수량 단위는 현재 SET 기준입니다.</div><div class="ops-cooking-menu-list">${menuRows}</div><div class="ops-settings-module-foot"><strong>BOT 자동 반영</strong><span>안내 문구와 메뉴 변경은 다음 요리 snapshot 갱신부터 Discord에 반영됩니다.</span></div></section>`;
}
