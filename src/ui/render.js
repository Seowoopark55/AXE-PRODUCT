...
function renderOnboardingProgress(state){
  ...
  return `<section class="ops-onboarding-progress"><div class="ops-onboarding-progress__copy"><strong>${phase==='ready'?'초기 설정 완료':'초기 설정'}</strong><span>${phase==='ready'?'현재 회사의 Discord 연결과 필수 설정이 준비됐습니다.':'순서대로 설정하면 같은 화면에서 바로 운영을 시작할 수 있습니다.'}</span></div><div class="ops-onboarding-steps">${labels.map(([key,no,label],i)=>`<span class="${i<activeIndex||phase==='ready'?'is-done':i===activeIndex?'is-current':''}"><b>${i<activeIndex||phase==='ready'?'✓':no}</b>${label}</span>`).join('')}</div></section>`;
}
