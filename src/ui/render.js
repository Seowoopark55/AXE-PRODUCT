...
function renderQuestions(state){
  ...
  }).join(''):`<div class="axe-questions-list-empty"><strong>아직 등록된 질문이 없습니다.</strong><span>궁금한 내용이 생기면 이곳에서 바로 질문을 남길 수 있습니다.</span></div>`;
  const unread=Number(counts.unread||0);
  return `<section class="axe-questions">${header}<section class="axe-question-summary">${summaryCards.map(([label,value,tone])=>`<article class="${tone}"><span>${label}</span><strong>${value}</strong><small>건</small></article>`).join('')}<button data-action="refresh-questions">새로고침</button></section>${unread?`<div class="axe-questions-alert"><strong>새 답변 ${unread}건</strong><span>확인하지 않은 답변이 있습니다.</span></div>`:''}<section class="axe-questions-board"><header><div><span>QUESTION BOARD</span><h2>최근 질문</h2></div><small>${Number(counts.total||items.length)}건</small></header><div class="axe-question-list">${list}</div></section><div class="axe-questions-note"><strong>운영 방식</strong><span>질문은 AXE PRODUCT 안에 저장됩니다. 운영자가 답변하면 <b>답변완료</b>로 바뀌고, Discord 계정이 연결되어 있으면 DM 알림도 시도합니다.</span></div></section>`;
}
...
function supportQuestionModal(state,m){
  ...
  const replyArea=canReply?`<form data-form="support-question-reply" class="runtime-modal-form support-question-reply"><input type="hidden" name="question_id" value="${esc(q.id)}"><label class="is-full"><span>${platform?'답변':'추가 질문'}</span><textarea name="body" maxlength="4000" rows="6" placeholder="${platform?'답변 내용을 입력하세요.':'추가로 궁금한 내용을 입력하세요.'}" required></textarea></label><div class="runtime-modal-hint is-full">${esc(note)}</div><footer><button type="button" class="runtime-btn-ghost" data-action="close-modal">닫기</button><button class="runtime-btn-primary" type="submit">${replyLabel}</button></footer></form>`:`<div class="runtime-modal-hint support-question-readonly">다른 멤버가 작성한 질문입니다. 답변은 함께 볼 수 있고, 별도 문의는 새 질문으로 등록해 주세요.</div><footer class="support-question-readonly-footer"><button type="button" class="runtime-btn-ghost" data-action="close-modal">닫기</button></footer>`;
