    ...
    <div class="workspace-shell">
      <aside class="sidebar">
        ...
        <nav class="sidebar-nav"><span class="sidebar-nav__label">회사 운영</span>
          ...
          <span class="sidebar-nav__label spaced">지원</span>${navItem(state,'questions','질문게시판')}<button class="nav-item nav-item--support" data-action="open-feedback"><span class="nav-item__icon">${icon('feedback')}</span><span>피드백 · 제보</span></button>
        </nav>
...
function navItem(state,key,label){ const badge=key==='questions'?Number(state.questionBoard?.counts?.unread||0):0; return `<button class="nav-item ${state.page===key?'is-active':''}" data-page="${key}"><span class="nav-item__icon">${icon(key)}</span><span>${label}</span>${badge?`<em class="nav-item__badge">${badge>99?'99+':badge}</em>`:''}</button>`; }
...
function renderDashboard(state){
  ...
  const attentionCount=dashboardFundPending(state).length+dashboardAccountPending(state).length+dashboardUnassignedAssets(state).length+Number(state.questionBoard?.counts?.unread||0)+platformQuestionCount+(connected?0:1);
...
// ============================================================
const OPS_PAGE_SIZE={questions:6,fund:8,members:8,assets:8,returns:8,accounts:8,cooking:9};
...
function renderQuestions(state){
  ...
  const filtered=filter==='all'?items:items.filter(item=>String(item.status||'pending')===filter);
  ...
  const list=paged.rows.length?paged.rows.map(item=>{
    ...
    return `<article class="axe-question-row ${item.unread?'is-unread':''}" data-action="open-question" data-question-id="${esc(item.id)}"><div class="axe-question-main"><div class="axe-question-title-line"><span class="axe-question-status ${meta.className}">${meta.label}</span>${unread}<strong>${esc(item.title)}</strong></div><div class="axe-question-meta"><span>${esc(item.author_name||'사용자')}</span><span>${esc(fmtDate(item.last_message_at||item.created_at,true))}</span><span>메시지 ${Math.max(1,Number(item.message_count||1))}개</span></div></div><div class="axe-question-actions"><button type="button" data-action="open-question" data-question-id="${esc(item.id)}">보기</button></div></article>`;
  }).join(''):`<div class="axe-questions-list-empty"><strong>조건에 맞는 질문이 없습니다.</strong><span>${filter==='all'?'궁금한 내용이 생기면 이곳에서 바로 질문을 남길 수 있습니다.':'다른 상태를 선택해 질문을 확인해 보세요.'}</span></div>`;
...
function renderModal(state){ const m=state.modal; if(!m)return ''; if(m.type==='setup-guide')return setupGuideLive(state); if(m.type==='setup-demo')return setupGuidePreview(state); if(m.type==='feedback')return feedbackModal(state); if(m.type==='support-question-create')return supportQuestionCreateModal(state); if(m.type==='support-question')return supportQuestionModal(state,m); if(m.type==='ledger')return ledgerModal(state,m); if(m.type==='ledger-correction')return ledgerCorrectionModal(state,m); if(m.type==='ledger-evidence')return ledgerEvidenceModal(state,m); if(m.type==='platform-subscription')return platformSubscriptionModal(state,m); if(m.type==='member')return memberModal(state,m); if(m.type==='asset')return assetModal(state,m); if(m.type==='account')return accountModal(state); if(m.type==='account-detail')return accountDetailModal(state,m); if(m.type==='create-company')return companyModal(); if(m.type==='discord-reconnect')return discordReconnectModal(state); if(m.type==='cooking-menu')return cookingMenuModal(state,m); return ''; }
...
function supportQuestionModal(state,m){
  ...
  const note=platform?'답변을 등록하면 자동으로 답변완료가 되고 작성자에게 Discord DM 알림을 시도합니다.':'추가 질문을 보내면 상태가 다시 답변대기로 변경됩니다.';
  const canReply=platform||q.viewer_can_reply!==false;
...
function setupGuidePreview(state){
  ...
  if(step===4){
    ...
    if(mode==='quick'){
      ...
      const rows=[]; if(modules.fund)rows.push(channelSelect('공금현황판','공금 현황판',demo.channels?.fund)); if(modules.ammo){rows.push(channelSelect('3시-총알','3시 총알 채널',demo.channels?.ammo3));rows.push(channelSelect('10시-총알','10시 총알 채널',demo.channels?.ammo10));} if(modules.outlaw)rows.push(channelSelect('전적-등록','무법 전적 등록 채널',demo.channels?.outlaw)); if(modules.modbook)rows.push(channelSelect('개조서','개조서 채널',demo.channels?.modbook)); if(modules.cooking)rows.push(channelSelect('요리-주문','요리 주문 채널',demo.channels?.cooking)); if(modules.assets)rows.push(channelSelect('계좌조회','계좌 조회 채널',demo.channels?.accountLookup));
