...
function renderLedgerRow(r,state){
  ...
  const type=`${kind}${r.direction?` · ${r.direction}`:''}`;
  return `<tr class="axe-fund-ledger-row"><td class="axe-fund-ledger-date" data-label="날짜"><strong>${y}.${m}.${d}</strong></td><td class="axe-fund-ledger-person" data-label="이름"><strong>${esc(who)}</strong></td><td class="axe-fund-ledger-account" data-label="계좌"><span>${esc(r.account||'—')}</span></td><td class="axe-fund-ledger-entry" data-label="내역" title="${esc(detail)}"><strong>${esc(detail)}</strong></td><td class="axe-fund-ledger-type ${r.direction==='수입'?'is-income':r.direction==='지출'?'is-expense':''}" data-label="구분">${esc(type)}</td><td class="axe-fund-ledger-money ${amount<0?'is-expense':'is-income'}" data-label="금액">${signedMoney(amount)}</td><td class="axe-fund-ledger-action" data-label="증빙">${evidenceControl}</td><td class="axe-fund-ledger-action" data-label="관리">${editControl}</td></tr>`;
}
...
function renderAssetBoard(state){
  ...
  if(q)rows=rows.filter(a=>`${a.asset_name||''} ${a.owner_name||''} ${a.asset_category||''} ${a.acquisition_method||''}`.toLowerCase().includes(q));
