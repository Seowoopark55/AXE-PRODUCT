...
function guideCenterModal(state){
  const current=guideSection(state);
  const currentMeta=GUIDE_SECTIONS.find(([key])=>key===current)||GUIDE_SECTIONS[0];
  return `<div class="axe-guide-dialog-backdrop" data-modal-backdrop>
    <section class="axe-guide-dialog" role="dialog" aria-modal="true" aria-label="AXE PRODUCT 사용 가이드">
      <aside class="axe-guide-dialog-rail">
        <div class="axe-guide-dialog-brand"><strong>AXE PRODUCT</strong><small>GUIDE CENTER</small></div>
        <div class="axe-guide-dialog-intro"><span>TUTORIAL</span><strong>기능별 사용 가이드</strong><p>Discord와 웹 콘솔의 실제 사용법을 기능별로 확인합니다.</p></div>
        <nav class="axe-guide-nav-list">${GUIDE_SECTIONS.map(([key,label,desc],index)=>`<button type="button" class="${current===key?'is-active':''}" data-guide-section="${esc(key)}"><b>${String(index+1).padStart(2,'0')}</b><span><strong>${esc(label)}</strong><small>${esc(desc)}</small></span></button>`).join('')}</nav>
        <button type="button" class="axe-guide-dialog-exit" data-action="close-modal">가이드 닫기</button>
      </aside>
      <main class="axe-guide-dialog-main">
        <header class="axe-guide-dialog-head"><div><span>운영 가이드</span><strong>${esc(currentMeta[1])}</strong><small>${esc(currentMeta[2])}</small></div><button type="button" data-action="close-modal" aria-label="가이드 닫기" title="가이드 닫기">×</button></header>
        <div class="axe-guide-dialog-scroll">${renderGuideBody(state,current)}</div>
      </main>
    </section>
  </div>`;
}
...
function setupGuideLive(state){
  ...
  const channelOptions=(selected='')=>`<option value="">채널 선택</option>${channels.map(c=>`<option value="${esc(c.channel_id)}" ${String(selected)===String(c.channel_id)?'selected':''}>#${esc(c.channel_name)}</option>`).join('')}`;
