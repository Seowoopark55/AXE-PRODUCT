...
function renderLedgerRow(r,state){
  ...
  const tags=[kind?`<span class="axe-fund-ledger-tag">${esc(kind)}</span>`:'',r.direction?`<span class="axe-fund-ledger-tag ${r.direction==='수입'?'is-income':'is-expense'}">${esc(r.direction)}</span>`:''].join('');
  const memo=r.memo?`<span class="axe-fund-ledger-note" title="${esc(r.memo)}">${esc(r.memo)}</span>`:'';
  return `<article class="axe-fund-ledger-row"><div class="axe-fund-ledger-date"><strong>${y}.${m}.${d}</strong></div><div class="axe-fund-ledger-person"><strong>${esc(who)}</strong>${r.account?`<span class="axe-fund-ledger-tag is-account">${esc(r.account)}</span>`:''}</div><div class="axe-fund-ledger-entry"><strong>${esc(title)}</strong><div class="axe-fund-ledger-meta">${tags}${memo}</div></div><div class="axe-fund-ledger-money ${amount<0?'is-expense':'is-income'}">${signedMoney(amount)}</div><div class="axe-fund-ledger-action">${evidenceControl}</div><div class="axe-fund-ledger-action">${editControl}</div></article>`;
}
...
function renderAssetBoard(state){
  ...
  const body=paged.rows.length?paged.rows.map(a=>`<article class="ops-lane-row ops-lane-row--asset"><div class="ops-lane-cell ops-lane-copy"><strong>${esc(a.owner_name||'미배정')}</strong></div><div class="ops-lane-cell ops-asset-identity"><strong>${esc(a.asset_name)}</strong>${a.acquisition_method?`<span class="ops-inline-tag">${esc(a.acquisition_method)}</span>`:''}</div><div class="ops-lane-cell"><span class="ops-lane-value">${esc(a.asset_category||'기타')}</span></div><div class="ops-lane-cell is-center"><span class="ops-mgmt-badge ${a.membership_id?'is-green':'is-amber'}">${a.membership_id?'사용중':'미배정'}</span></div><div class="ops-lane-cell is-center"><button class="ops-mgmt-action ${a.membership_id?'':'is-assign'}" data-action="edit-asset" data-asset-id="${esc(a.id)}">${a.membership_id?'상세':'배정'}</button></div></article>`).join(''):empty('조건에 맞는 자산이 없습니다.');
...
function renderCookingMenuSettings(state){
  ...
  const menuRows=paged.rows.length?paged.rows.map(row=>{
    ...
    return `<article class="ops-cooking-menu-row ${row.enabled===false?'is-disabled':''}"><div class="ops-cooking-menu-main"><div><strong>${esc(row.label||row.type_key)}</strong><span class="ops-cooking-order-badge">#${Number(row.sort_order||0)}</span></div>${showDetail?`<small>${esc(detail)}</small>`:''}</div><div class="ops-cooking-menu-price"><strong>${money(Number(row.price_per_set||0))}<em>/SET</em></strong></div><button type="button" class="runtime-power ${row.enabled===false?'is-off':'is-on'}" data-action="toggle-cooking-menu" data-type-key="${esc(row.type_key)}" ${!admin?'disabled':''}>${icon('power')}<span>${row.enabled===false?'OFF':'ON'}</span></button><button type="button" class="ops-mgmt-action" data-action="edit-cooking-menu" data-type-key="${esc(row.type_key)}" ${!admin?'disabled':''}>수정</button></article>`;
