    ...
    <header class="global-header"><div class="global-header__inner">
      ...
      <div class="global-account runtime-account-picker ${state.accountMenuOpen?'is-open':''}">
        <button type="button" class="runtime-account-trigger" data-action="toggle-account-menu" aria-expanded="${state.accountMenuOpen?'true':'false'}" aria-haspopup="menu">
          <span><strong>${esc(userDisplayName(state))}</strong><small>${esc(ROLE_LABEL[membership?.role] || (state.platformAdmin?'PLATFORM OWNER':'-'))}</small></span><b>⌄</b>
        </button>
...
function renderDataPager(key,paged,noun='건'){
  if(!paged||paged.total<=0)return '';
  const buttons=paged.totalPages>1?`<div class="ops-data-pager__buttons"><button type="button" data-action="list-page" data-list-key="${esc(key)}" data-list-page="${Math.max(1,paged.page-1)}" ${paged.page<=1?'disabled':''}>이전</button><span>${paged.page} / ${paged.totalPages}</span><button type="button" data-action="list-page" data-list-key="${esc(key)}" data-list-page="${Math.min(paged.totalPages,paged.page+1)}" ${paged.page>=paged.totalPages?'disabled':''}>다음</button></div>`:'';
  return `<footer class="ops-data-pager"><span><strong>${paged.start}-${paged.end}</strong> / ${paged.total}${esc(noun)}</span>${buttons}</footer>`;
}
...
function renderQuestions(state){
  ...
  const header=pageHeader('SUPPORT','질문게시판','기본 사용법은 각 기능 패널에서 확인하고, 막히는 내용만 질문으로 남겨주세요.',createAction);
  if(board.error){
...
function renderSuggestions(state){
  ...
  const header=pageHeader('PRIVATE SUPPORT','건의게시판','작성한 건의는 본인과 AXE ONE 운영자만 볼 수 있습니다.',createAction);
  if(board.error){
...
function renderLedgerRow(r,state){
  ...
  return `<article class="axe-fund-ledger-row"><div class="axe-fund-ledger-date"><strong>${m}.${d}</strong><span>${y}</span></div><div class="axe-fund-ledger-person"><strong>${esc(who)}</strong><span>${esc(r.account||'—')}</span></div><div class="axe-fund-ledger-entry"><div><strong>${esc(title)}</strong></div><small>${esc(sub||'—')}</small></div><div class="axe-fund-ledger-money ${amount<0?'is-expense':'is-income'}">${signedMoney(amount)}</div><div class="axe-fund-ledger-action">${evidenceControl}</div><div class="axe-fund-ledger-action">${editControl}</div></article>`;
}
...
function renderReturns(state){
  ...
  const body=paged.rows.length?paged.rows.map(r=>`<article class="ops-lane-row ops-lane-row--return"><div class="ops-lane-cell ops-lane-copy"><strong>${esc(r.asset_name)}</strong><span>이전 보유자 ${esc(r.owner_name)}</span></div><div class="ops-lane-cell ops-lane-copy"><strong>${r.note?'수동 반납':'반납 처리'}</strong><span>${esc(r.note||'기록 보존')}</span></div><div class="ops-lane-cell ops-lane-copy"><strong>${esc(r.checker_name||'SYSTEM')}</strong><span>${esc(r.processed_at||fmtDate(r.created_at,true))}</span></div></article>`).join(''):empty('반납 기록이 없습니다.');
  return `<section class="ops-mgmt-board"><div class="ops-mgmt-board-head"><div><h2>반납 내역</h2><p>퇴사 자동반납과 수동 반납 이력을 기록으로 보존합니다.</p></div><span>${rows.length}건</span></div><div class="ops-lane-head ops-lane-head--return"><span>자산</span><span>처리</span><span>확인자</span></div><div class="ops-mgmt-list">${body}</div>${renderDataPager('returns',paged,'건')}</section>`;
}
...
function renderPlatform(state){
  ...
  const body=paged.rows.length?paged.rows.map(r=>`<article class="platform-company-row"><div class="platform-company-copy"><strong>${esc(r.company_name||'회사')}</strong><span>${esc(r.guild_name||'Discord 미연결')} · ${Number(r.member_count||0)}명</span></div><div><span class="ops-mgmt-badge ${platformStatusClass(r.effective_status||r.subscription_status)}">${esc(platformStatusLabel(r.effective_status||r.subscription_status))}</span></div><div class="platform-plan"><strong>${esc(platformPlanLabel(r.plan))}</strong><span>${r.days_remaining==null?'기간 제한 없음':Number(r.days_remaining)<0?`${Math.abs(Number(r.days_remaining))}일 초과`:`D-${Number(r.days_remaining)}`}</span></div><div class="platform-period"><strong>${platformDate(r.ends_at)}</strong><span>${r.starts_at?`${platformDate(r.starts_at)} 시작`:'시작일 미설정'}</span></div><div class="platform-owner"><strong>${esc(r.owner_name||'OWNER 미확인')}</strong><span>회사 OWNER</span></div><div class="is-center"><button class="ops-mgmt-action" data-action="edit-platform-subscription" data-company-id="${esc(r.company_id)}">관리</button></div></article>`).join(''):empty('조건에 맞는 회사가 없습니다.');
  const companyBoard=`<section class="platform-board"><div class="ops-mgmt-toolbar"><div class="ops-mgmt-filters"><label class="ops-mgmt-search">${icon('search')}<input data-platform-query value="${esc(state.platformQuery||'')}" placeholder="회사 · OWNER · Discord 검색"></label><select class="ops-mgmt-select" data-platform-status><option value="all" ${filter==='all'?'selected':''}>상태 전체</option><option value="trial" ${filter==='trial'?'selected':''}>체험</option><option value="active" ${filter==='active'?'selected':''}>사용중</option><option value="paused" ${filter==='paused'?'selected':''}>정지</option><option value="expired" ${filter==='expired'?'selected':''}>만료</option></select></div><span class="ops-mgmt-toolbar-note">한 화면 최대 ${OPS_PAGE_SIZE.platform}개</span></div><div class="ops-mgmt-meta platform-company-meta"><span><strong>${rows.length}</strong>개 검색 결과</span><span>페이지 이동으로 목록 높이를 고정합니다.</span></div><div class="platform-company-head"><span>회사</span><span>상태</span><span>플랜</span><span>이용 종료</span><span>OWNER</span><span>관리</span></div><div class="platform-company-list">${body}</div>${renderDataPager('platform',paged,'개')}</section>`;
  ...
  return `<div class="platform-page">${pageHeader('PLATFORM OWNER','서비스 관리','회사·고객 문의·비공개 건의를 탭으로 나눠 화면 높이가 회사 수에 따라 늘어나지 않도록 관리합니다.',`<button class="ops-action-secondary" data-action="refresh-platform">${icon('refresh')}<span>새로고침</span></button>`)}${summary([['전체 회사',`${all.length}개`,'등록 회사',''],['이용 가능',`${active}개`,'체험 · 사용중 · 무제한','is-positive'],['고객 질문',`${questionOpen}건`,'대기 · 확인중','is-warning'],['건의 · 제보',`${suggestionOpen}건`,'1:1 비공개','is-warning']])}${tabs}<div class="platform-service-view">${content}</div></div>`;
}
...
function renderCookingMenuSettings(state){
  ...
  const query=String(state.cookingQuery||'').trim().toLowerCase();
  const status=String(state.cookingStatus||'all');
  ...
  const menuRows=paged.rows.length?paged.rows.map(row=>`<article class="ops-cooking-menu-row ${row.enabled===false?'is-disabled':''}"><div class="ops-cooking-menu-main"><strong>${esc(row.label||row.type_key)}</strong><span>${esc(row.short_label||row.label||row.type_key)}${row.detail?` · ${esc(row.detail)}`:''}</span></div><div class="ops-cooking-menu-price"><strong>${money(Number(row.price_per_set||0))}</strong><span>SET당</span></div><div class="ops-cooking-menu-order"><strong>${Number(row.sort_order||0)}</strong><span>순서</span></div><button type="button" class="runtime-power ${row.enabled===false?'is-off':'is-on'}" data-action="toggle-cooking-menu" data-type-key="${esc(row.type_key)}" ${!admin?'disabled':''}>${icon('power')}<span>${row.enabled===false?'OFF':'ON'}</span></button><button type="button" class="ops-mgmt-action" data-action="edit-cooking-menu" data-type-key="${esc(row.type_key)}" ${!admin?'disabled':''}>수정</button></article>`).join(''):`<div class="ops-cooking-menu-empty"><strong>조건에 맞는 요리 메뉴가 없습니다.</strong><span>${all.length?'검색어나 상태 필터를 바꿔보세요.':admin?'오른쪽 메뉴 추가 버튼으로 첫 주문 품목을 등록해 주세요.':'회사 관리자가 메뉴를 등록하면 여기에 표시됩니다.'}</span></div>`;
  return `<div class="ops-cooking-settings">${guide}<section class="ops-settings-board ops-cooking-menu-board"><div class="ops-settings-board-head ops-cooking-menu-head"><div><h2>요리 주문 메뉴</h2><p>활성 메뉴만 Discord 주문창에 표시됩니다.</p></div><div class="ops-cooking-menu-tools"><span class="ops-cooking-menu-count"><strong>${active}</strong> / ${all.length} 사용 중</span><button type="button" class="ops-cooking-add" data-action="open-cooking-menu" ${!admin?'disabled':''}>${icon('plus')}<span>메뉴 추가</span></button></div></div><div class="ops-cooking-filterbar"><label class="ops-mgmt-search">${icon('search')}<input data-cooking-query value="${esc(state.cookingQuery||'')}" placeholder="메뉴명 · 설명 검색" autocomplete="off"></label><select class="ops-mgmt-select" data-cooking-status><option value="all" ${status==='all'?'selected':''}>전체 메뉴</option><option value="enabled" ${status==='enabled'?'selected':''}>사용 중</option><option value="disabled" ${status==='disabled'?'selected':''}>숨김</option></select><span>${rows.length}개 검색 결과</span></div><div class="ops-cooking-menu-list">${menuRows}</div>${renderDataPager('cooking',paged,'개')}<div class="ops-settings-module-foot"><strong>BOT 자동 반영</strong><span>안내와 메뉴 변경은 다음 요리 snapshot부터 Discord에 반영됩니다.</span></div></section></div>`;
}
...
function setupGuidePreview(state){
  ...
  if(step===4){
    ...
    if(mode==='quick'){
      const category=demo.categoryName||'AXE ONE';
