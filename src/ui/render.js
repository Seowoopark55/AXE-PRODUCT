    ...
    <div class="workspace-shell">
      <aside class="sidebar">
        ...
        <footer class="sidebar-footer"><div class="connection-status ${connected?'':'is-off'}"><i></i><div><strong>Discord ${connected?'연결됨':'미연결'}</strong><small>${esc(state.discordConnection?.guild_name || '연결 필요')}</small></div></div>
          <div class="runtime-sidebar-tools"><span class="environment-pill"><i></i>PRODUCT STAGING</span><button class="mini-icon" data-action="refresh" title="새로고침">${icon('refresh')}</button></div>
        </footer>
      </aside>
