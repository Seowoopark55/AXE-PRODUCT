...
function icon(name) {
  const icons = {
    ...
    layout:'<rect x="3" y="4" width="18" height="16" rx="2"/><path d="M8 4v16M8 10h13M3 14h5"/>',
...
function renderStartupLoading() {
  return `<section class="runtime-auth runtime-auth--startup"><div class="runtime-startup-card"><span class="runtime-startup-spinner" aria-hidden="true"></span><div><strong>AXE ONE</strong><p>회사 정보를 불러오는 중입니다.</p></div></div></section>`;
}

function renderOfflineLoading() {
  return `<section class="runtime-auth runtime-auth--startup"><div class="runtime-startup-card runtime-startup-card--offline"><span class="runtime-offline-dot" aria-hidden="true"></span><div><strong>AXE ONE</strong><p>네트워크 연결이 없습니다. 운영 데이터는 오프라인에 저장하지 않습니다.</p></div><button class="runtime-btn-ghost" data-action="refresh">다시 연결</button></div></section>`;
}

function renderEnvironmentMissing() {
  return `<section class="runtime-auth"><div class="runtime-auth-card"><strong>AXE ONE</strong><h1>STAGING 환경 설정이 필요합니다.</h1><p>VITE_SUPABASE_URL / VITE_SUPABASE_PUBLISHABLE_KEY를 Vercel 환경변수에 설정해 주세요.</p></div></section>`;
}
...
function renderLogin(state) {
  ...
  const privacy=state.discordAuthMinimal
    ? `<span class="runtime-access-privacy is-minimal"><i>✓</i>Discord 기본 프로필만 확인 · 이메일 권한 요청 없음</span>`
    : `<span class="runtime-access-privacy"><i>◇</i>Discord 인증 후 계정 식별 정보로 회사 권한을 확인합니다.</span>`;
  return `<section class="runtime-auth runtime-auth--access"><div class="runtime-access-shell">
    <section class="runtime-access-brand-panel">
      <div class="runtime-access-brand-top"><span class="runtime-access-private"><i></i>PRIVATE ACCESS</span><div class="product-brand product-brand--login"><div><strong>AXE ONE</strong><small>OPERATIONS CONSOLE</small></div></div></div>
      ...
      <div class="runtime-access-steps"><div><b>01</b><span><strong>IDENTITY</strong><small>Discord 계정으로 본인 확인</small></span></div><div><b>02</b><span><strong>ACCESS</strong><small>회사 멤버십 또는 초대권 확인</small></span></div><div><b>03</b><span><strong>CONSOLE</strong><small>승인된 회사 운영 공간으로 연결</small></span></div></div>
    </section>
    <section class="runtime-access-panel">
      <div class="runtime-access-panel-head"><span>SECURE SIGN IN</span><h2>${hasInvite?'초대받은 계정으로 계속':'AXE ONE에 접속'}</h2><p>${hasInvite?'초대 링크가 확인되었습니다. Discord 계정으로 본인을 확인하면 초대권을 자동으로 확인합니다.':'기존 구성원은 Discord로 로그인하면 등록된 회사로 바로 연결됩니다.'}</p></div>
      ...
      <button class="runtime-access-discord" data-action="discord-login" ${state.loading?'disabled':''}><span class="runtime-access-discord-mark">D</span><strong>${hasInvite?'Discord로 초대 확인':'Discord로 계속하기'}</strong><i>→</i></button>
      <div class="runtime-access-divider"><span>또는</span></div>
      ${inviteOpen?`<form class="runtime-access-invite-form" data-form="invite-login"><label><span>초대 코드</span><div><input data-invite-code-input name="invite_code" maxlength="96" placeholder="XXXX-XXXX-XXXX" autocomplete="off" required><button type="submit">코드로 계속</button></div></label><button type="button" class="runtime-access-collapse" data-action="hide-invite-code">입력 취소</button></form>`:`<button type="button" class="runtime-access-invite-toggle" data-action="show-invite-code"><span>초대 코드를 가지고 있어요</span><i>+</i></button>`}
      ${state.inviteError?`<div class="runtime-access-inline-error">${esc(state.inviteError)}</div>`:''}
      <div class="runtime-access-trust">${privacy}<small>로그인만으로 회사 데이터가 공개되지 않습니다. 등록된 멤버십 또는 유효한 초대권이 있어야 진입할 수 있습니다.</small></div>
    </section>
  </div><footer class="runtime-access-footer"><span>AXE ONE</span><i></i><span>AUTHORIZED COMPANY OPERATIONS ONLY</span></footer></section>`;
}

function onboardingDiscordIdentity(state) {
  const user=state.session?.user||{};
  const meta=user.user_metadata||{};
  const id=String(meta.provider_id||meta.sub||user.identities?.find?.(item=>String(item?.provider||'').toLowerCase().includes('discord'))?.identity_data?.sub||'').trim();
...
function firstRunContent(discord,{testMode=false,focus='',memberCheck=''}={}) {
  // Test Center keeps the historical scenario simulator isolated from the live access gate.
  ...
  return `
    <header class="runtime-first-run__head"><span>AXE ONE TEST SCENARIO</span><h1>소속 회사를 확인하지 못했습니다.</h1><p>테스트 센터에서 기존 최초 접속 분기 동작을 점검합니다.</p></header>
    <div class="runtime-first-run__grid">
      <article class="runtime-onboarding-card runtime-onboarding-card--primary runtime-first-run__create${createFocus}"><span>NEW COMPANY · TEST</span><h2>새 회사 등록 시뮬레이션</h2><p>PLATFORM OWNER 테스트 센터에서만 기존 회사 생성 흐름을 확인합니다.</p>
        <div class="runtime-first-run__new-flow"><span>회사 이름</span><i>→</i><span>Discord 연결</span><i>→</i><span>초기설정</span></div>
        <button class="runtime-login-button runtime-first-run__primary-action" type="button" data-action="${createAction}">테스트 회사 등록 흐름</button>
        <small class="runtime-first-run__auto">실제 미소속 사용자의 진입 화면에서는 자유 회사 생성을 제공하지 않습니다.</small>
      </article>
      <article class="runtime-onboarding-card runtime-first-run__member${memberFocus}"><div class="runtime-first-run__member-top"><span>TEAM MEMBER · TEST</span><em>멤버십 확인</em></div><h2>기존 멤버 연결 시뮬레이션</h2><p>Discord 계정이 회사 멤버로 등록된 경우 자동 연결되는 흐름을 확인합니다.</p>
        ...
        <button class="runtime-btn-ghost runtime-first-run__check-button" type="button" data-action="${checkAction}">등록 확인하기</button>${waiting}
      </article>
    </div>`;
}
...
function renderOnboarding(state) {
  ...
  return `<section class="runtime-auth runtime-auth--access runtime-auth--access-gate"><div class="runtime-access-gate">
    ...
    <footer class="runtime-access-gate-footer"><span>링크를 알고 있는 것만으로는 회사 데이터에 접근할 수 없습니다.</span><button type="button" data-action="logout">다른 Discord 계정 사용</button></footer>
  </div></section>`;
}
  ...
  return `<div class="runtime-app runtime-app--${esc(state.page||'fund')}">
    <header class="global-header"><div class="global-header__inner">
      <button type="button" class="global-home-zone" data-action="go-dashboard" aria-label="홈으로 이동">
        <span class="product-brand product-brand--home"><span class="product-brand__copy"><strong>AXE ONE</strong><small>OPERATIONS CONSOLE</small></span></span>
      </button>
    ...
    <div class="workspace-shell">
      <aside class="sidebar">
        ...
        <nav class="sidebar-nav"><span class="sidebar-nav__label">회사 운영</span>
          ...
          <span class="sidebar-nav__label spaced">지원</span>${navItem(state,'questions','질문게시판')}${navItem(state,'suggestions','건의게시판')}
        </nav>
...
function navItem(state,key,label){ const badge=key==='questions'?Number(state.questionBoard?.counts?.unread||0):key==='suggestions'?Number(state.suggestionBoard?.counts?.unread||0):0; return `<button class="nav-item ${state.page===key?'is-active':''}" data-page="${key}"><span class="nav-item__icon">${icon(key)}</span><span>${label}</span>${badge?`<em class="nav-item__badge">${badge>99?'99+':badge}</em>`:''}</button>`; }
...
function renderPage(state) {
  ...
  if (state.page === 'questions') return renderQuestions(state);
  if (state.page === 'suggestions') return renderSuggestions(state);
...
// ============================================================
// LAYOUT STUDIO · PLATFORM OWNER
// ============================================================
function renderLayoutStudio(state){
  const p=state.layoutStudio||{};
  const textPreset=detectLayoutStudioPreset(p,'text');
  const densityPreset=detectLayoutStudioPreset(p,'density');
  const widthPreset=detectLayoutStudioPreset(p,'width');
  const actionPreset=detectLayoutStudioPreset(p,'action');
  const choice=(type,value,label,active)=>`<button type="button" class="${active?'is-active':''}" data-action="layout-preset" data-layout-type="${type}" data-layout-value="${value}">${label}</button>`;
  const step=(key,label,help,value,unit,amount)=>`<div class="layout-studio-adjust"><div><strong>${label}</strong><small>${help}</small></div><div class="layout-studio-stepper"><button type="button" data-action="layout-adjust" data-layout-key="${key}" data-layout-delta="-${amount}">−</button><span>${Number(value).toFixed(String(amount).includes('.')?1:0)}${unit}</span><button type="button" data-action="layout-adjust" data-layout-key="${key}" data-layout-delta="${amount}">+</button></div></div>`;
  const dirty=state.layoutStudioDirty;
  return `<div class="layout-studio-page">
    ${pageHeader('PLATFORM OWNER','레이아웃 스튜디오','','')}
    <div class="layout-studio-note">${icon('layout')}<span><b>코드 대신 화면을 보면서 조정합니다.</b> 지금 변경은 현재 브라우저에서 즉시 미리보기 되고, 저장해야 새로고침 후에도 유지됩니다.</span></div>
    <div class="layout-studio-grid">
      <section class="layout-studio-panel">
        <header><div><strong>빠른 조정</strong><span>자주 만질 값만 쉬운 말로 묶었습니다.</span></div><span class="layout-studio-saved ${dirty?'is-dirty':''}">${dirty?'저장되지 않은 변경':'저장된 설정'}</span></header>
        <div class="layout-studio-section"><div class="layout-studio-section__head"><div><strong>글자 크기</strong><span>표 안의 제목·내용·필터·상태·관리 글자를 함께 조절합니다.</span></div><span class="layout-studio-current">${textPreset==='custom'?'직접 설정':textPreset==='small'?'작게':textPreset==='default'?'기본':textPreset==='comfortable'?'조금 크게':'크게'}</span></div><div class="layout-studio-choice">${choice('text','small','작게',textPreset==='small')}${choice('text','default','기본',textPreset==='default')}${choice('text','comfortable','조금 크게',textPreset==='comfortable')}${choice('text','large','크게',textPreset==='large')}</div></div>
        <div class="layout-studio-section"><div class="layout-studio-section__head"><div><strong>행 간격</strong><span>표 상자 폭은 유지하면서 헤더·데이터 행의 숨 쉴 공간만 조절합니다.</span></div><span class="layout-studio-current">${densityPreset==='custom'?'직접 설정':densityPreset==='compact'?'촘촘':densityPreset==='default'?'기본':'여유'}</span></div><div class="layout-studio-choice is-three">${choice('density','compact','촘촘',densityPreset==='compact')}${choice('density','default','기본',densityPreset==='default')}${choice('density','relaxed','여유',densityPreset==='relaxed')}</div></div>
        <div class="layout-studio-section"><div class="layout-studio-section__head"><div><strong>표 너비</strong><span>현재 균형을 크게 흔들지 않도록 작은 범위만 제공합니다.</span></div><span class="layout-studio-current">${widthPreset==='custom'?`${p.railWidth}px`:widthPreset==='compact'?'조금 좁게':widthPreset==='default'?'기본 유지':'조금 넓게'}</span></div><div class="layout-studio-choice is-three">${choice('width','compact','조금 좁게',widthPreset==='compact')}${choice('width','default','기본 유지',widthPreset==='default')}${choice('width','wide','조금 넓게',widthPreset==='wide')}</div></div>
        <div class="layout-studio-section"><div class="layout-studio-section__head"><div><strong>상태 · 관리 요소</strong><span>상태 배지와 상세/관리 버튼의 글자와 최소 폭을 조절합니다.</span></div><span class="layout-studio-current">${actionPreset==='custom'?'직접 설정':actionPreset==='compact'?'작게':actionPreset==='default'?'기본':'크게'}</span></div><div class="layout-studio-choice is-three">${choice('action','compact','작게',actionPreset==='compact')}${choice('action','default','기본',actionPreset==='default')}${choice('action','large','크게',actionPreset==='large')}</div></div>
        <button type="button" class="layout-studio-advanced-toggle" data-action="layout-toggle-advanced"><span>세부 설정 ${state.layoutStudioAdvanced?'접기':'열기'}</span><b>${state.layoutStudioAdvanced?'−':'+'}</b></button>
        ${state.layoutStudioAdvanced?`<div class="layout-studio-advanced">${step('headerFont','헤더 글자','이름 · 역할 · 상태 같은 열 이름',p.headerFont,'px',.1)}${step('primaryFont','주요 내용','이름 · 금액처럼 가장 먼저 읽는 값',p.primaryFont,'px',.1)}${step('secondaryFont','보조 내용','역할 · 계좌 · 분류 같은 값',p.secondaryFont,'px',.1)}${step('controlFont','필터 · 검색','탭 · 필터 · 검색창 글자',p.controlFont,'px',.1)}${step('statusFont','상태 글자','활동 · 승인 같은 상태 배지',p.statusFont,'px',.1)}${step('actionFont','관리 글자','상세 · 관리 버튼 글자',p.actionFont,'px',.1)}${step('headerHeight','헤더 높이','열 이름이 들어가는 줄 높이',p.headerHeight,'px',1)}${step('rowHeight','데이터 행 높이','한 줄 데이터의 높이',p.rowHeight,'px',1)}${step('gap','칸 사이 간격','열과 열 사이의 기본 간격',p.gap,'px',1)}${step('cellPad','좌우 여백','표 안쪽 좌우 여백',p.cellPad,'px',1)}${step('railWidth','표 너비','Standard 운영 표의 전체 폭',p.railWidth,'px',4)}</div>`:''}
        <div class="layout-studio-actions"><button type="button" data-action="layout-reset-default">기본값</button><div><button type="button" data-action="layout-revert">저장값으로 되돌리기</button><button type="button" class="is-primary" data-action="layout-save">현재 설정 저장</button></div></div>
      </section>
      <section class="layout-studio-preview"><header><div><strong>실시간 미리보기</strong><span>실제 운영 테이블 규격과 같은 값을 사용합니다.</span></div><span>${p.railWidth}px</span></header><div class="layout-studio-preview-body"><div class="layout-preview-board"><div class="layout-preview-toolbar"><div class="layout-preview-tabs"><span>전체 1</span><span>활동 1</span><span>퇴사 0</span></div><div class="layout-preview-search">멤버 검색</div></div><div class="layout-preview-head"><span>이름</span><span>Discord</span><span>역할</span><span>입사일</span><span>상태</span><span>관리</span></div><div class="layout-preview-row"><span class="layout-preview-primary">영포티</span><span class="layout-preview-secondary">영포티</span><span class="layout-preview-secondary">대표</span><span class="layout-preview-secondary">미설정</span><span class="layout-preview-status">활동</span><span class="layout-preview-action">상세</span></div></div><p class="layout-studio-preview-tip"><b>TIP</b> · 저장 전에도 실제 멤버·계좌·자산·공금·요리 화면에 같은 값이 즉시 적용됩니다. 다른 화면을 확인한 뒤 다시 돌아와 저장할 수 있습니다.</p></div></section>
    </div>
  </div>`;
}
...
function renderModuleSettings(state,channels){ const visible=(state.modules||[]).filter(m=>MODULE_UI[m.module_key]).sort((a,b)=>MODULE_ORDER.indexOf(a.module_key)-MODULE_ORDER.indexOf(b.module_key)); const resetBusy=['reset_requested','resetting'].includes(String(state.onboardingStatus?.status||'')); return `<form id="settings-active-form" data-form="settings-modules" class="ops-settings-board ops-settings-modules"><div class="ops-settings-board-head"><div><h2>기능 설정</h2></div><span><strong>${visible.filter(m=>m.enabled).length}</strong> / ${visible.length} 사용 중</span></div><div class="ops-settings-module-list runtime-module-list">${visible.map(m=>renderModuleRow(m,channels,resetBusy)).join('')}</div></form>`; }
...
function supportQuestionModal(state,m){
  ...
  const note=platform?'답변은 AXE ONE 운영자만 등록할 수 있습니다. 등록하면 자동으로 답변완료가 되고 작성자에게 Discord DM 알림을 시도합니다.':'작성자는 자신의 질문에만 추가 질문을 남길 수 있습니다. 추가 질문을 보내면 상태가 다시 답변대기로 변경됩니다.';
...
function suggestionThreadModal(state,m){
  ...
  return modalShell(q.title||'건의','작성자와 AXE ONE 운영자만 볼 수 있는 1:1 비공개 스레드입니다.',`<div class="support-question-detail suggestion-thread-detail"><div class="support-question-detail-head"><div><span class="suggestion-category ${cat.className}">${cat.label}</span><span class="axe-question-status ${meta.className}">${meta.label}</span>${q.unread?'<em>NEW</em>':''}</div><div><strong>${esc(q.company_name||companyDisplayName(state))}</strong><span>${esc(q.author_name||'사용자')} · ${esc(fmtDate(q.created_at))}</span></div>${statusActions}</div><div class="support-thread">${thread}</div>${replyArea}</div>`,true);
}
...
function memberRegisterModal(state){
  return modalShell('멤버 등록','팀원이 전달한 Discord ID로 현재 회사에 먼저 등록합니다.',`<form data-form="member-register" class="runtime-modal-form" autocomplete="off">
    <div class="runtime-modal-hint is-full"><strong>팀원이 첫 접속 화면에서 복사한 Discord ID를 붙여넣어 주세요.</strong><br>현재 연결된 Discord 서버의 실제 멤버인지 확인한 뒤 등록합니다. 등록 전에는 팀원이 AXE ONE에 진입할 수 없습니다.</div>
    ...
    <label class="is-full">AXE 권한<select name="role"><option value="member" selected>일반 멤버</option><option value="manager">매니저</option><option value="admin">관리자</option></select><small>OWNER 권한은 이 등록 화면에서 부여하지 않습니다.</small></label>
...
const SETUP_CHANNEL_CATEGORY_ORDER = ['AXE ONE · 무법지대','AXE ONE · 회사운영','AXE ONE · 편의기능'];
function setupChannelCategoryName(key){
...
function setupChannelCategoryNames(plan,getKey){
  return SETUP_CHANNEL_CATEGORY_ORDER.filter(name=>plan.some(row=>setupChannelCategoryName(getKey(row))===name));
}
function setupChannelCategoryGroups(plan,getKey,rowRenderer){
  return setupChannelCategoryNames(plan,getKey).map(name=>{
    const rows=plan.filter(row=>setupChannelCategoryName(getKey(row))===name);
    return `<section class="setup-demo-category-group"><div class="setup-demo-category-head"><span><strong>${esc(name)}</strong><small>관련 기능을 한곳에 정리합니다.</small></span><em>${rows.length}개 채널</em></div><div class="setup-demo-create-list">${rows.map(rowRenderer).join('')}</div></section>`;
  }).join('');
}
...
function setupGuidePreview(state){
  ...
  if(step===0){ body=`<div class="setup-demo-hero"><span class="setup-demo-kicker">AXE ONE SETUP GUIDE</span><h2>처음 설정도, 순서대로 하면 어렵지 않습니다.</h2><p>필요한 것만 하나씩 안내하고 완료된 단계는 자동으로 확인합니다. 지금 체험하는 내용은 실제 회사 설정에 저장되지 않습니다.</p><div class="setup-demo-facts"><span><b>약 2–3분</b> 예상 소요시간</span><span><b>6단계</b> 핵심 설정만 진행</span><span><b>0건</b> 실제 데이터 변경</span></div></div>`; nextLabel='체험 시작'; }
  ...
  if(step===4){
    ...
    if(mode==='quick'){
      const categoryNames=setupChannelCategoryNames(channelPlan,row=>row[0]);
      const groupedPlan=setupChannelCategoryGroups(channelPlan,row=>row[0],generatedRow);
      const builder=channelPlan.length?`<div class="setup-demo-builder"><div class="setup-demo-builder-head"><div><strong>추천 구성</strong><span>기능 성격에 따라 ${categoryNames.length}개 카테고리로 자동 정리합니다.</span></div><em>${channelPlan.length}개 채널</em></div>${groupedPlan}${demo.channelsGenerated?`<div class="setup-demo-generation-result"><div class="setup-demo-generation-title"><i>✓</i><span><strong>생성 체험 완료</strong><small>실제 Discord에는 아무것도 생성되지 않았습니다.</small></span></div><ul>${categoryNames.map(name=>`<li><b>✓</b><span><strong>${esc(name)}</strong><small>카테고리 생성</small></span></li>`).join('')}${channelPlan.map(([,label,,value])=>`<li><b>✓</b><span><strong>#${esc(value)}</strong><small>${esc(label)}에 자동 연결</small></span></li>`).join('')}</ul></div>`:`<div class="setup-demo-builder-note"><b>PREVIEW</b><span>아래 <strong>이 구성으로 생성 체험</strong>을 누르면 추천 카테고리와 채널 배치를 미리 볼 수 있습니다.</span></div>`}</div>`:`<div class="setup-demo-empty"><strong>자동 생성이 필요한 채널이 없습니다.</strong><span>선택한 기능은 별도 Discord 채널이 필요하지 않습니다.</span></div>`;
      body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>채널도 AXE가 보기 좋게 준비합니다.</h2><p>기능별 추천 카테고리에 자동 배치하고, 채널 이름은 생성 전에 자유롭게 수정할 수 있습니다.</p>${modePicker}${builder}</div>`;
    }else{
  ...
  return `<div class="setup-demo-backdrop"><section class="setup-demo-shell" role="dialog" aria-modal="true" aria-label="초기설정 가이드 체험"><aside class="setup-demo-rail"><div class="setup-demo-brand"><strong>AXE ONE</strong><small>GUIDED SETUP · PREVIEW</small></div><div class="setup-demo-preview-badge"><i></i><span>테스트 설정</span><small>실제 데이터 변경 없음</small></div><nav>${rail}</nav><button type="button" class="setup-demo-exit" data-action="close-modal">체험 닫기</button></aside><main class="setup-demo-main"><header><div><span>초기설정 가이드 체험</span><strong>${step+1} / ${steps.length}</strong></div><button type="button" data-action="close-modal" aria-label="닫기">×</button></header><div class="setup-demo-content ${scrollStep?'is-scroll-step':''}">${body}</div><footer>${step>0&&step<6?`<button type="button" class="setup-demo-back" data-action="setup-demo-back">이전</button>`:'<span></span>'}<div><button type="button" class="setup-demo-restart" data-action="setup-demo-restart">처음부터</button>${primary}</div></footer></main></section></div>`;
}
...
function setupGuideLive(state){
  ...
  const plan=[];
  if(modules.fund)plan.push({key:'fund',label:'공금 관리',placeholder:'공금현황판',value:guide.generatedChannels?.fund||'공금현황판',type:'text',categoryName:setupChannelCategoryName('fund')});
  if(modules.ammo){plan.push({key:'ammo3',label:'총알 관리 · 3시',placeholder:'3시-총알',value:guide.generatedChannels?.ammo3||'3시-총알',categoryName:setupChannelCategoryName('ammo3')});plan.push({key:'ammo10',label:'총알 관리 · 10시',placeholder:'10시-총알',value:guide.generatedChannels?.ammo10||'10시-총알',categoryName:setupChannelCategoryName('ammo10')});}
  if(modules.pinball)plan.push({key:'pinball',label:'핀볼 모집',placeholder:'핀볼-모집',value:guide.generatedChannels?.pinball||'핀볼-모집',categoryName:setupChannelCategoryName('pinball')});
  if(modules.outlaw)plan.push({key:'outlaw',label:'무법지대 전적',placeholder:'전적-등록',value:guide.generatedChannels?.outlaw||'전적-등록',categoryName:setupChannelCategoryName('outlaw')});
  if(modules.modbook)plan.push({key:'modbook',label:'개조서 조회 · 가격',placeholder:'개조서',value:guide.generatedChannels?.modbook||'개조서',categoryName:setupChannelCategoryName('modbook')});
  if(modules.cooking)plan.push({key:'cooking',label:'요리 주문',placeholder:'요리-주문',value:guide.generatedChannels?.cooking||'요리-주문',categoryName:setupChannelCategoryName('cooking')});
  if(modules.assets)plan.push({key:'accountLookup',label:'계좌 조회',placeholder:'계좌조회',value:guide.generatedChannels?.accountLookup||'계좌조회',type:'text',categoryName:setupChannelCategoryName('accountLookup')});
  ...
  if(step===2){body=`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 02</span><h2>누가 운영하고, 누가 사용하는지 정합니다.</h2><p>Discord 역할을 기준으로 AXE 관리자와 일반 멤버를 구분합니다.</p><div class="setup-demo-role-grid"><label class="setup-demo-field"><span>관리자 역할</span><select data-guide-role="adminRoleId">${roleOptions(guide.adminRoleId)}</select><small>멤버 · 공금 · 자산 · 회사 설정 관리</small></label><label class="setup-demo-field"><span>일반 멤버 역할</span><select data-guide-role="memberRoleId">${roleOptions(guide.memberRoleId)}</select><small>회사 기능을 사용하는 일반 구성원</small></label></div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-save-roles" ${guide.adminRoleId&&guide.memberRoleId?'':'disabled'}>저장하고 다음</button>`;}
  ...
  if(step===4){const mode=guide.channelMode==='direct'?'direct':'quick';const modePicker=`<div class="setup-demo-channel-mode"><button type="button" class="${mode==='quick'?'is-active':''}" data-action="setup-guide-channel-mode" data-mode="quick"><b>⚡</b><span><strong>빠른 설정</strong><small>필요한 채널을 AXE가 생성하고 바로 연결합니다.</small></span></button><button type="button" class="${mode==='direct'?'is-active':''}" data-action="setup-guide-channel-mode" data-mode="direct"><b>↗</b><span><strong>직접 연결</strong><small>이미 사용하는 Discord 채널을 선택합니다.</small></span></button></div>`;const permissionCard=guide.permissionIssue==='manage_channels'?`<div class="setup-guide-permission-card"><div class="setup-guide-permission-icon">!</div><div class="setup-guide-permission-copy"><strong>채널 생성 권한이 필요합니다.</strong><span>${esc(guide.permissionMessage||'AXE가 채널을 만들려면 Discord의 채널 관리 권한 승인이 필요합니다.')}</span><small>현재 연결된 Discord 서버만 고정해서 권한 승인을 다시 진행합니다. 관리자 전체 권한은 요청하지 않습니다.</small></div><div class="setup-guide-permission-actions"><button type="button" class="is-primary" data-action="setup-guide-reapprove-channels">권한 다시 승인</button><button type="button" data-action="setup-guide-channel-mode" data-mode="direct">기존 채널 직접 연결</button></div></div>`:'';if(mode==='quick'){body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>채널도 AXE가 준비할 수 있습니다.</h2><p>기능별 추천 카테고리에 자동 배치하고, 채널 이름은 생성 전에 자유롭게 수정할 수 있습니다.</p>${modePicker}${permissionCard}${plan.length?`<div class="setup-demo-builder"><div class="setup-demo-builder-head"><div><strong>생성할 구성</strong><span>기능 성격에 따라 ${setupChannelCategoryNames(plan,row=>row.key).length}개 카테고리로 자동 정리합니다.</span></div><em>${plan.length}개 채널</em></div>${setupChannelCategoryGroups(plan,row=>row.key,generatedRow)}<div class="setup-demo-builder-note"><b>LIVE</b><span>생성하면 Discord 서버에 추천 카테고리와 채널이 만들어지고 AXE 기능에 자동 연결됩니다.</span></div></div>`:`<div class="setup-demo-empty"><strong>생성할 채널이 없습니다.</strong><span>선택한 기능은 별도의 Discord 채널이 필요하지 않습니다.</span></div>`}</div>`;primary=plan.length?`<button type="button" class="setup-demo-primary setup-demo-primary--generate" data-action="setup-guide-create-channels">이 구성으로 생성</button>`:`<button type="button" class="setup-demo-primary" data-action="setup-guide-next">다음</button>`;}else{body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>기존 채널을 그대로 연결할 수 있습니다.</h2><p>서버 구조를 바꾸지 않고 AXE 기능만 기존 채널에 연결합니다.</p>${modePicker}<div class="setup-demo-channel-list">${directRows||`<div class="setup-demo-empty"><strong>연결할 채널이 없습니다.</strong><span>바로 다음 단계로 진행할 수 있습니다.</span></div>`}</div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-save-direct-channels">저장하고 다음</button>`;}}
  if(step===5){const filterRoleName=roles.find(r=>String(r.role_id)===String(guide.memberFilterRoleId))?.role_name||'역할';body=`<div class="setup-demo-quest setup-demo-quest--members"><span class="setup-demo-quest-no">QUEST 05</span><h2>기존 멤버를 한 번에 등록할 수 있습니다.</h2><p>Discord 역할로 회사 인원만 걸러낸 뒤 AXE 권한을 일괄 부여합니다.</p><div class="setup-demo-member-toolbar"><label><span>Discord 역할로 대상 찾기</span><select data-guide-member-filter>${roleOptions(guide.memberFilterRoleId)}</select><small>게스트가 많은 서버에서도 원하는 역할의 멤버만 불러옵니다.</small></label><label><span>AXE에서 부여할 권한</span><select data-guide-member-target-role><option value="member" ${guide.memberTargetRole!=='admin'?'selected':''}>일반 멤버</option><option value="admin" ${guide.memberTargetRole==='admin'?'selected':''}>관리자</option></select><small>선택한 Discord 멤버에게 부여할 AXE 권한입니다.</small></label></div>${guide.memberListLoaded?`<div class="setup-demo-member-summary"><div><strong>${esc(filterRoleName)} 역할</strong><span>${Number(guide.memberScanCount||candidates.length)}명 확인 · ${candidates.length}명 표시</span></div><button type="button" data-action="setup-guide-select-members">${allSelected?'현재 목록 선택 해제':'현재 목록 전체 선택'}</button></div><div class="setup-demo-member-list">${memberRows||`<div class="setup-demo-empty"><strong>해당 역할의 등록 가능한 멤버가 없습니다.</strong><span>BOT 계정과 기존 등록 멤버는 제외될 수 있습니다.</span></div>`}</div>`:`<div class="setup-demo-empty"><strong>역할을 선택해 멤버를 불러오세요.</strong><span>Discord 서버 인원이 많아도 선택한 역할만 필터링합니다.</span></div>`}${guide.memberImportDone?guide.memberImportSkipped?`<div class="setup-demo-member-result is-skipped"><i>→</i><div><strong>멤버 등록은 나중에 진행합니다.</strong><span>Discord 우클릭 → 앱 → AXE 멤버 등록으로 개별 추가할 수 있습니다.</span></div></div>`:`<div class="setup-demo-member-result"><i>✓</i><div><strong>선택한 멤버 등록을 완료했습니다.</strong><span>멤버 관리에서 등록 결과를 확인할 수 있습니다.</span></div></div>`:`<div class="setup-demo-member-actions"><span><b>${selected.size}명 선택됨</b></span><button type="button" data-action="setup-guide-skip-members">나중에 개별 등록</button></div>`}</div>`;primary=guide.memberImportDone?`<button type="button" class="setup-demo-primary" data-action="setup-guide-next">다음</button>`:guide.memberListLoaded?`<button type="button" class="setup-demo-primary setup-demo-primary--members" data-action="setup-guide-import-members" ${selected.size?'':'disabled'}>선택 멤버 등록</button>`:`<button type="button" class="setup-demo-primary" data-action="setup-guide-load-members" ${guide.memberFilterRoleId?'':'disabled'}>멤버 불러오기</button>`;}
  if(step===6){body=`<div class="setup-demo-complete"><div class="setup-demo-complete-mark">✓</div><span class="setup-demo-kicker">SETUP COMPLETE</span><h2>운영 준비가 완료되었습니다.</h2><p>설정한 역할·기능·채널이 현재 회사에 적용되었습니다.</p><div class="setup-demo-next-task"><span>NEXT</span><div><strong>이제 AXE ONE를 바로 사용할 수 있습니다.</strong><small>추가 멤버는 Discord 우클릭 → 앱 → AXE 멤버 등록으로 언제든 추가할 수 있습니다.</small></div></div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-finish">운영 콘솔 시작</button>`;}
  const scrollStep=step===4||step===5;
  return `<div class="setup-demo-backdrop"><section class="setup-demo-shell" role="dialog" aria-modal="true" aria-label="초기설정 가이드"><aside class="setup-demo-rail"><div class="setup-demo-brand"><strong>AXE ONE</strong><small>GUIDED SETUP</small></div><div class="setup-demo-preview-badge is-live"><i></i><span>실제 설정</span><small>현재 회사에 단계별 적용</small></div><nav>${rail}</nav><button type="button" class="setup-demo-exit" data-action="close-modal">가이드 닫기</button></aside><main class="setup-demo-main"><header><div><span>초기설정 가이드</span><strong>${step+1} / ${steps.length}</strong></div><button type="button" data-action="close-modal" aria-label="닫기">×</button></header><div class="setup-demo-content ${scrollStep?'is-scroll-step':''}">${body}</div><footer>${step>0&&step<6?`<button type="button" class="setup-demo-back" data-action="setup-guide-back">이전</button>`:'<span></span>'}<div>${primary}</div></footer></main></section></div>`;
}
