...
function dashboardAttentionItems(state){
  ...
  const questionPending=Number(state.questionBoard?.counts?.pending||0);
  const questionChecking=Number(state.questionBoard?.counts?.checking||0);
  if(questionPending||questionChecking) items.push({tone:'amber',icon:'questions',title:`질문 답변 ${questionPending+questionChecking}건`,desc:`답변대기 ${questionPending} · 확인중 ${questionChecking}`,page:'questions',action:'답변하기'});
...
function renderDashboard(state){
  ...
  const attentionCount=dashboardFundPending(state).length+dashboardAccountPending(state).length+dashboardUnassignedAssets(state).length+Number(state.questionBoard?.counts?.pending||0)+Number(state.questionBoard?.counts?.checking||0)+(connected?0:1);
...
function questionAuthorName(state,item){
  const id=String(item?.owner_id||'');
  const row=(state.memberships||[]).find(m=>String(m.discord_user_id||'')===id);
  return row?.display_name||row?.discord_display_name||'Discord 사용자';
}
function renderQuestions(state){
  const board=state.questionBoard||{};
  const counts=board.counts||{};
  const configured=Boolean(board.configured&&board.forum_url);
  const items=Array.isArray(board.items)?board.items:[];
  const admin=canAdmin(state);
  const forumAction=configured?`<a class="axe-questions-primary" href="${esc(board.forum_url)}" target="_blank" rel="noopener noreferrer">Discord 질문게시판 열기 ↗</a>`:(admin?`<button class="axe-questions-primary" data-action="open-setup-guide">초기설정에서 연결</button>`:'');
  const header=pageHeader('SUPPORT','질문게시판','기본 사용법은 각 Discord 기능 패널에서 확인하고, 막히는 내용만 질문으로 남겨주세요.',forumAction);
  if(board.error){
  ...
  if(!configured){
    return `<section class="axe-questions">${header}<div class="axe-questions-empty"><span>QUESTION BOARD</span><strong>Discord 질문게시판 연결이 필요합니다.</strong><p>${admin?'초기설정에서 질문게시판 포럼 채널을 자동 생성하거나 기존 포럼 채널을 연결해 주세요.':'관리자가 질문게시판 채널을 연결하면 이곳에서 바로 사용할 수 있습니다.'}</p>${admin?`<button data-action="open-setup-guide">초기설정 열기</button>`:''}</div></section>`;
  }
  ...
  const list=items.length?items.map(item=>{
    ...
    const actions=admin?`<div class="axe-question-admin-actions">${item.status!=='pending'?`<button data-action="question-status" data-thread-id="${esc(item.id)}" data-status="pending">대기로</button>`:''}${item.status!=='checking'?`<button data-action="question-status" data-thread-id="${esc(item.id)}" data-status="checking">확인중</button>`:''}${item.status!=='complete'?`<button class="is-complete" data-action="question-status" data-thread-id="${esc(item.id)}" data-status="complete">답변완료</button>`:''}</div>`:'';
    return `<article class="axe-question-row"><div class="axe-question-main"><div class="axe-question-title-line"><span class="axe-question-status ${meta.className}">${meta.label}</span><strong>${esc(item.title)}</strong></div><div class="axe-question-meta"><span>${esc(questionAuthorName(state,item))}</span><span>${esc(fmtDate(item.created_at,true))}</span><span>답글 ${Math.max(0,Number(item.message_count||0)-1)}개</span></div></div><div class="axe-question-actions">${actions}<a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">글 보기 ↗</a></div></article>`;
  }).join(''):`<div class="axe-questions-list-empty"><strong>아직 등록된 질문이 없습니다.</strong><span>Discord 질문게시판에서 새 질문을 작성하면 이곳에 표시됩니다.</span></div>`;
  return `<section class="axe-questions">${header}<section class="axe-question-summary">${summaryCards.map(([label,value,tone])=>`<article class="${tone}"><span>${label}</span><strong>${value}</strong><small>건</small></article>`).join('')}<button data-action="refresh-questions">새로고침</button></section><section class="axe-questions-board"><header><div><span>RECENT QUESTIONS</span><h2>최근 질문</h2></div><small>${esc(board.forum_name||'질문게시판')} · ${Number(counts.total||items.length)}건</small></header><div class="axe-question-list">${list}</div></section><div class="axe-questions-note"><strong>운영 방식</strong><span>새 글은 답변대기로 집계됩니다. 관리자는 답변 후 <b>답변완료</b>로 변경할 수 있고, 완료 처리 시 작성자에게 Discord DM 알림을 시도합니다.</span></div></section>`;
}
...
function setupGuidePreview(state){
  ...
  const channelPlan=[['questionForum','질문게시판 · 문의/답변','질문게시판',generated.questionForum||'질문게시판','질문게시판']];
...
function setupGuideLive(state){
  ...
  const forumChannels=(state.discordChannels||[]).filter(c=>Number(c.channel_type)===15||String(c.channel_type||'').toLowerCase().includes('forum'));
  const textChannels=channels.filter(c=>!forumChannels.some(f=>String(f.channel_id)===String(c.channel_id)));
  ...
  const channelOptions=(selected='',type='text')=>{const source=type==='forum'?forumChannels:textChannels;return `<option value="">${type==='forum'?'포럼 채널':'채널'} 선택</option>${source.map(c=>`<option value="${esc(c.channel_id)}" ${String(selected)===String(c.channel_id)?'selected':''}>#${esc(c.channel_name)}</option>`).join('')}`;};
  ...
  const plan=[{key:'questionForum',label:'질문게시판 · 문의/답변',placeholder:'질문게시판',value:guide.generatedChannels?.questionForum||'질문게시판',type:'forum'}];
  ...
  const directRows=plan.map(row=>`<label class="setup-demo-field"><span>${esc(row.label)}</span><select data-guide-direct-channel="${esc(row.key)}">${channelOptions(guide.directChannels?.[row.key]||'',row.type||'text')}</select></label>`).join('');
