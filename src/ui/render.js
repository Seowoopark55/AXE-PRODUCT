...
function renderCompanyBanner(state) {
  const name = companyDisplayName(state);
  const image = state.companyBannerUrl
    ? `<img src="${esc(state.companyBannerUrl)}" alt="" onerror="this.remove();this.parentElement.classList.add('is-fallback')">`
    : '';
  return `<section class="company-hero ${state.companyBannerUrl?'has-image':'is-fallback'}">${image}<div class="company-hero__shade"></div><div class="company-hero__copy"><span>COMPANY</span><strong>${esc(name)}</strong></div></section>`;
}
...
function renderFundLedger(state) {
  ...
  return `<section class="axe-fund-history"><header class="axe-fund-history-head"><div><h2>공금내역</h2><p>필요한 정보만 빠르게 확인하고, 상세 작업은 행에서 바로 처리합니다.</p></div><div class="axe-fund-history-head-actions"><select class="axe-fund-history-select axe-fund-history-select--month" data-fund-ledger-month>${monthOptions(state.fundMonth,12)}</select><button class="axe-fund-primary axe-fund-primary--ledger" data-action="open-ledger">수입·지출 등록</button></div></header>
    ...
    <div class="axe-fund-table-wrap"><table class="axe-fund-table"><colgroup><col class="is-date"><col class="is-name"><col class="is-entry"><col class="is-money"><col class="is-evidence"><col class="is-manage"></colgroup><thead><tr><th>날짜</th><th>이름</th><th>내역</th><th>금액</th><th>증빙</th><th>관리</th></tr></thead><tbody>${rows.length?rows.map(r=>renderLedgerRow(r,state)).join(''):`<tr class="is-empty"><td colspan="6"><div class="axe-fund-history-empty">조건에 맞는 공금내역이 없습니다.</div></td></tr>`}</tbody></table></div></section></section>`;
}
function renderLedgerRow(r,state){ const amount=Number(r.amount||0); const source=r.request_id?'승인':'직접'; const isWeeklyPayment=r.entry_type==='payment'; const title=isWeeklyPayment?'주간공금':(r.category||'기타'); const sub=[isWeeklyPayment?'공금납부':r.ledger_type,r.direction,r.memo].filter(Boolean).join(' · '); const current=(state.memberships||[]).find(m=>m.id===r.membership_id); const who=current?.display_name||r.member_display_name||'—'; const key=dateKey(r.ledger_date); const [y,m,d]=key.split('-'); return `<tr><td data-label="날짜" class="axe-fund-table-date"><strong>${m}.${d}</strong><span>${y}</span></td><td data-label="이름" class="axe-fund-table-who"><strong>${esc(who)}</strong><span>${esc(r.account||'—')}</span></td><td data-label="내역" class="axe-fund-table-entry"><div><strong>${esc(title)}</strong><span class="axe-fund-history-kind axe-fund-history-kind--${r.request_id?'approval':'manual'}">${source}</span></div><small>${esc(sub||'—')}</small></td><td data-label="금액" class="axe-fund-table-money ${amount<0?'is-expense':'is-income'}">${signedMoney(amount)}</td><td data-label="증빙" class="axe-fund-table-action">${r.evidence_path?`<button class="axe-fund-history-action is-evidence" data-action="open-evidence" data-evidence-path="${esc(r.evidence_path)}">증빙</button>`:'<span class="axe-fund-history-no-evidence">—</span>'}</td><td data-label="관리" class="axe-fund-table-action">${r.can_edit?`<button class="axe-fund-history-action" data-action="edit-ledger" data-entry-id="${esc(r.id)}">수정</button>`:'<span class="axe-fund-history-no-evidence">—</span>'}</td></tr>`; }
...
function renderMembers(state){
  ...
  const memberRows=rows.length?rows.map(m=>{
    ...
    return `<tr>
      <td data-label="이름" class="ops-table-primary"><strong>${esc(m.display_name||'멤버')}</strong><small>${m.alias_name?'별칭 적용':'기본 표시명'}</small></td>
      <td data-label="역할"><strong>${esc(ROLE_KO[m.role]||m.role)}</strong><small>역할</small></td>
      <td data-label="입사일"><strong>${esc(hire)}</strong><small>입사일</small></td>
      <td data-label="상태"><span class="ops-mgmt-badge ${m.status==='active'?'is-green':m.status==='left'?'is-red':'is-amber'}">${memberStatus(m.status)}</span></td>
      <td data-label="관리"><button class="ops-mgmt-action" data-action="edit-member" data-membership-id="${esc(m.id)}">상세</button></td>
    </tr>`;
  }).join(''):`<tr class="is-empty"><td colspan="5">${empty('조건에 맞는 멤버가 없습니다.')}</td></tr>`;
  return `<div class="ops-mgmt-page ops-mgmt-page--members">${pageHeader('MEMBERS','멤버 관리','회사 구성원의 역할·활동 상태·실제 입사일을 관리합니다.','')}${summary([['전체 멤버',`${all.length}명`,'누적 등록',''],['활동 중',`${active.length}명`,'현재 회사','is-positive'],['관리 권한',`${admins.length}명`,'대표 · 관리자','is-warning'],['퇴사',`${left.length}명`,'기록 유지','']])}<div class="ops-mgmt-section-head ops-mgmt-section-head--solo"><div><strong>멤버 현황</strong><span>멤버 등록은 Discord에서 대상 우클릭 → 앱 → AXE 멤버 등록으로 처리합니다.</span></div></div><section class="ops-mgmt-board"><div class="ops-mgmt-toolbar"><div class="ops-mgmt-segments">${segment(state,'all','전체',all.length)}${segment(state,'active','활동',active.length)}${segment(state,'left','퇴사',left.length,true)}</div><div class="ops-mgmt-filters"><select class="ops-mgmt-select" data-member-role><option value="">역할 전체</option>${['owner','admin','manager','member'].map(r=>`<option value="${r}" ${state.memberRole===r?'selected':''}>${ROLE_KO[r]}</option>`).join('')}</select><label class="ops-mgmt-search">${icon('search')}<input data-member-query value="${esc(state.memberQuery||'')}" placeholder="이름 · 별칭 · 역할 검색" autocomplete="off"></label></div></div><div class="ops-mgmt-meta"><span><strong>${rows.length}</strong>명 표시</span><span>표 보기</span></div><div class="ops-data-table-wrap"><table class="ops-data-table ops-member-table"><colgroup><col class="is-name"><col class="is-role"><col class="is-hire"><col class="is-status"><col class="is-manage"></colgroup><thead><tr><th>이름</th><th>역할</th><th>입사일</th><th>상태</th><th>관리</th></tr></thead><tbody>${memberRows}</tbody></table></div></section></div>`;
}
...
function renderAssetBoard(state){ let rows=(state.assetsSnapshot?.assets||[]).slice(); const q=(state.assetQuery||'').toLowerCase(); if(state.assetCategory)rows=rows.filter(a=>a.asset_category===state.assetCategory); if(state.assetStatus)rows=rows.filter(a=>(a.membership_id?'사용중':'미배정')===state.assetStatus); if(q)rows=rows.filter(a=>`${a.asset_name||''} ${a.owner_name||''} ${a.asset_category||''}`.toLowerCase().includes(q)); const cats=[...new Set((state.assetsSnapshot?.assets||[]).map(a=>a.asset_category).filter(Boolean))]; const body=rows.length?rows.map(a=>`<tr><td data-label="보유자"><strong>${esc(a.owner_name||'미배정')}</strong><small>현재 보유자</small></td><td data-label="자산" class="ops-table-primary"><strong>${esc(a.asset_name)}</strong><small>자산명</small></td><td data-label="분류"><strong>${esc(a.asset_category||'기타')}</strong><small>분류</small></td><td data-label="상태"><span class="ops-mgmt-badge ${a.membership_id?'is-green':'is-amber'}">${a.membership_id?'사용중':'미배정'}</span></td><td data-label="관리"><button class="ops-mgmt-action ${a.membership_id?'':'is-assign'}" data-action="edit-asset" data-asset-id="${esc(a.id)}">${a.membership_id?'상세':'배정'}</button></td></tr>`).join(''):`<tr class="is-empty"><td colspan="5">${empty('조건에 맞는 자산이 없습니다.')}</td></tr>`; return `<section class="ops-mgmt-board"><div class="ops-mgmt-toolbar"><div class="ops-mgmt-filters"><label class="ops-mgmt-search">${icon('search')}<input data-asset-query value="${esc(state.assetQuery||'')}" placeholder="자산명 · 보유자 검색"></label><select class="ops-mgmt-select" data-asset-category><option value="">분류 전체</option>${cats.map(c=>`<option ${state.assetCategory===c?'selected':''}>${esc(c)}</option>`).join('')}</select><select class="ops-mgmt-select" data-asset-status><option value="">상태 전체</option><option ${state.assetStatus==='사용중'?'selected':''}>사용중</option><option ${state.assetStatus==='미배정'?'selected':''}>미배정</option></select></div></div><div class="ops-mgmt-meta"><span><strong>${rows.length}</strong>개 표시</span><span>퇴사자의 자산은 자동으로 미배정되어 다시 배정할 수 있습니다.</span></div><div class="ops-data-table-wrap"><table class="ops-data-table ops-asset-table"><colgroup><col class="is-holder"><col class="is-asset"><col class="is-category"><col class="is-status"><col class="is-manage"></colgroup><thead><tr><th>보유자</th><th>자산</th><th>분류</th><th>상태</th><th>관리</th></tr></thead><tbody>${body}</tbody></table></div></section>`; }
function renderReturns(state){ const rows=state.assetsSnapshot?.returns||[]; const body=rows.length?rows.map(r=>`<tr><td data-label="자산" class="ops-table-primary"><strong>${esc(r.asset_name)}</strong><small>이전 보유자 ${esc(r.owner_name)}</small></td><td data-label="처리"><strong>${r.note?'수동 반납':'반납 처리'}</strong><small>${esc(r.note||'기록 보존')}</small></td><td data-label="확인자"><strong>${esc(r.checker_name||'SYSTEM')}</strong><small>${esc(r.processed_at||fmtDate(r.created_at,true))}</small></td></tr>`).join(''):`<tr class="is-empty"><td colspan="3">${empty('반납 기록이 없습니다.')}</td></tr>`; return `<section class="ops-mgmt-board"><div class="ops-mgmt-board-head"><div><h2>반납 내역</h2><p>퇴사 자동반납과 수동 반납 이력을 기록으로 보존합니다.</p></div><span>${rows.length}건</span></div><div class="ops-data-table-wrap"><table class="ops-data-table ops-return-table"><colgroup><col class="is-asset"><col class="is-process"><col class="is-checker"></colgroup><thead><tr><th>자산</th><th>처리</th><th>확인자</th></tr></thead><tbody>${body}</tbody></table></div></section>`; }
