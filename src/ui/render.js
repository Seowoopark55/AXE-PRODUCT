...
function renderFundReview(state) {
  ...
  return `<section class="axe-fund-card axe-fund-subview axe-fund-subview--review"><header class="axe-fund-card-head"><div><h2>납부 검수</h2></div><span class="axe-fund-chip">${open.length}건</span></header><div class="axe-fund-review-list">${rows}</div>${renderDataPager('fundReview',paged,'건')}</section>`;
}
...
function renderReturns(state){
  ...
  return `<section class="ops-mgmt-board"><div class="ops-mgmt-board-head"><div><h2>반납 내역</h2></div><span>${rows.length}건</span></div><div class="ops-lane-head ops-lane-head--return"><span>자산</span><span>처리</span><span>확인자</span></div><div class="ops-mgmt-list">${body}</div>${renderDataPager('returns',paged,'건')}</section>`;
}
...
function renderCookingMenuSettings(state){
  ...
  return `<div class="ops-cooking-settings">${guide}<section class="ops-settings-board ops-cooking-menu-board"><div class="ops-settings-board-head ops-cooking-menu-head"><div><h2>요리 주문 메뉴</h2></div><div class="ops-cooking-menu-tools"><span class="ops-cooking-menu-count"><strong>${active}</strong> / ${all.length} 사용 중</span><button type="button" class="ops-cooking-add" data-action="open-cooking-menu" ${!admin?'disabled':''}>${icon('plus')}<span>메뉴 추가</span></button></div></div><div class="ops-cooking-filterbar"><label class="ops-mgmt-search">${icon('search')}<input data-cooking-query value="${esc(state.cookingQuery||'')}" placeholder="메뉴명 · 설명 검색" autocomplete="off"></label><select class="ops-mgmt-select" data-cooking-status><option value="all" ${status==='all'?'selected':''}>전체 메뉴</option><option value="enabled" ${status==='enabled'?'selected':''}>사용 중</option><option value="disabled" ${status==='disabled'?'selected':''}>숨김</option></select><span>${rows.length}개 검색 결과</span></div><div class="ops-cooking-menu-list">${menuRows}</div>${renderDataPager('cooking',paged,'개')}</section></div>`;
}
