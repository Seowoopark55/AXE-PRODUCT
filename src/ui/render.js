...
function renderStartupLoading() {
  return `<section class="runtime-auth runtime-auth--startup"><div class="runtime-startup-card"><span class="runtime-startup-spinner" aria-hidden="true"></span><div><strong>LAC ONE</strong><p>회사 정보를 불러오는 중입니다.</p></div></div></section>`;
}

function renderOfflineLoading() {
  return `<section class="runtime-auth runtime-auth--startup"><div class="runtime-startup-card runtime-startup-card--offline"><span class="runtime-offline-dot" aria-hidden="true"></span><div><strong>LAC ONE</strong><p>네트워크 연결이 없습니다. 운영 데이터는 오프라인에 저장하지 않습니다.</p></div><button class="runtime-btn-ghost" data-action="refresh">다시 연결</button></div></section>`;
}

function renderEnvironmentMissing() {
  return `<section class="runtime-auth"><div class="runtime-auth-card"><strong>LAC ONE</strong><h1>STAGING 환경 설정이 필요합니다.</h1><p>VITE_SUPABASE_URL / VITE_SUPABASE_PUBLISHABLE_KEY를 Vercel 환경변수에 설정해 주세요.</p></div></section>`;
}
...
function firstRunContent(discord,{testMode=false,focus='',memberCheck=''}={}) {
  ...
  return `
    <header class="runtime-first-run__head"><span>LAC ONE START</span><h1>소속 회사를 확인하지 못했습니다.</h1><p>새 회사를 등록할 대표인지, 이미 LAC ONE을 이용 중인 회사의 팀원인지 선택해 주세요.</p></header>
    <div class="runtime-first-run__grid">
      <article class="runtime-onboarding-card runtime-onboarding-card--primary runtime-first-run__create${createFocus}"><span>NEW COMPANY</span><h2>새 회사 등록 시작</h2><p>아직 LAC ONE을 이용 중인 회사가 없다면 새 운영 공간을 만들고 초기설정을 바로 시작합니다.</p>
      ...
      <article class="runtime-onboarding-card runtime-first-run__member${memberFocus}"><div class="runtime-first-run__member-top"><span>TEAM MEMBER</span><em>멤버 등록 필요</em></div><h2>이미 이용 중인 회사의 팀원입니다</h2><p>대표 또는 관리자에게 아래 Discord 계정을 <b>멤버로 먼저 등록</b>해 달라고 요청해 주세요. 등록되기 전에는 LAC ONE에 진입할 수 없습니다.</p>
  ...
  return `<div class="runtime-app runtime-app--${esc(state.page||'fund')}">
    <header class="global-header"><div class="global-header__inner">
      <button type="button" class="global-home-zone" data-action="go-dashboard" aria-label="홈으로 이동">
        <span class="product-brand product-brand--home"><span class="product-brand__copy"><strong>LAC ONE</strong><small>OPERATIONS CONSOLE</small></span></span>
      </button>
...
function supportQuestionModal(state,m){
  ...
  const note=platform?'답변은 LAC ONE 운영자만 등록할 수 있습니다. 등록하면 자동으로 답변완료가 되고 작성자에게 Discord DM 알림을 시도합니다.':'작성자는 자신의 질문에만 추가 질문을 남길 수 있습니다. 추가 질문을 보내면 상태가 다시 답변대기로 변경됩니다.';
...
function suggestionThreadModal(state,m){
  ...
  return modalShell(q.title||'건의','작성자와 LAC ONE 운영자만 볼 수 있는 1:1 비공개 스레드입니다.',`<div class="support-question-detail suggestion-thread-detail"><div class="support-question-detail-head"><div><span class="suggestion-category ${cat.className}">${cat.label}</span><span class="axe-question-status ${meta.className}">${meta.label}</span>${q.unread?'<em>NEW</em>':''}</div><div><strong>${esc(q.company_name||companyDisplayName(state))}</strong><span>${esc(q.author_name||'사용자')} · ${esc(fmtDate(q.created_at))}</span></div>${statusActions}</div><div class="support-thread">${thread}</div>${replyArea}</div>`,true);
}
...
function memberRegisterModal(state){
  return modalShell('멤버 등록','팀원이 전달한 Discord ID로 현재 회사에 먼저 등록합니다.',`<form data-form="member-register" class="runtime-modal-form" autocomplete="off">
    <div class="runtime-modal-hint is-full"><strong>팀원이 첫 접속 화면에서 복사한 Discord ID를 붙여넣어 주세요.</strong><br>현재 연결된 Discord 서버의 실제 멤버인지 확인한 뒤 등록합니다. 등록 전에는 팀원이 LAC ONE에 진입할 수 없습니다.</div>
    ...
    <label class="is-full">LAC ONE 권한<select name="role"><option value="member" selected>일반 멤버</option><option value="manager">매니저</option><option value="admin">관리자</option></select><small>OWNER 권한은 이 등록 화면에서 부여하지 않습니다.</small></label>
...
function setupGuidePreview(state){
  ...
  if(step===0){ body=`<div class="setup-demo-hero"><span class="setup-demo-kicker">LAC ONE SETUP GUIDE</span><h2>처음 설정도, 순서대로 하면 어렵지 않습니다.</h2><p>필요한 것만 하나씩 안내하고 완료된 단계는 자동으로 확인합니다. 지금 체험하는 내용은 실제 회사 설정에 저장되지 않습니다.</p><div class="setup-demo-facts"><span><b>약 2–3분</b> 예상 소요시간</span><span><b>6단계</b> 핵심 설정만 진행</span><span><b>0건</b> 실제 데이터 변경</span></div></div>`; nextLabel='체험 시작'; }
  ...
  if(step===4){
    ...
    if(mode==='quick'){
      ...
      body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>채널도 LAC ONE이 준비할 수 있습니다.</h2><p>추천 구성을 그대로 사용하거나, 카테고리와 채널명을 회사 스타일에 맞게 바꾼 뒤 생성합니다.</p>${modePicker}${builder}</div>`;
    }else{
      ...
      body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>기존 채널을 그대로 연결할 수도 있습니다.</h2><p>이미 서버 구조가 잡혀 있다면 새 채널을 만들지 않고 기존 채널을 기능에 연결합니다.</p>${modePicker}<div class="setup-demo-channel-list">${rows.length?rows.join(''):`<div class="setup-demo-empty"><strong>채널 설정이 필요한 기능이 없습니다.</strong><span>바로 다음 단계로 진행할 수 있습니다.</span></div>`}</div><div class="setup-demo-tip"><strong>직접 연결</strong><span>기존 서버 구조는 그대로 유지하고 LAC ONE 기능만 연결합니다.</span></div></div>`;
    }
  }
  if(step===5){
    const selectedVisible=visibleMembers.filter(m=>selectedMembers.has(m.id)).length;
    body=`<div class="setup-demo-quest setup-demo-quest--members"><span class="setup-demo-quest-no">QUEST 05</span><h2>멤버도 한 번에 등록할 수 있습니다.</h2><p>서버 인원이 많아도 Discord 역할로 먼저 걸러낸 뒤, 필요한 사람만 선택해서 LAC ONE 권한을 일괄 부여합니다.</p><div class="setup-demo-member-toolbar"><label><span>Discord 역할로 대상 찾기</span><select data-setup-member-filter><option value="member" ${filter==='member'?'selected':''}>${esc(memberRole)} · 일반 멤버 역할 (6)</option><option value="admin" ${filter==='admin'?'selected':''}>${esc(adminRole)} · 관리자 역할 (2)</option><option value="guest" ${filter==='guest'?'selected':''}>손님 · 게스트 (10)</option></select><small>게스트가 많은 서버에서도 실제 회사 인원만 빠르게 찾습니다.</small></label><label><span>LAC ONE에서 부여할 권한</span><select data-setup-member-target-role><option value="member" ${demo.memberTargetRole!=='admin'?'selected':''}>일반 멤버</option><option value="admin" ${demo.memberTargetRole==='admin'?'selected':''}>관리자</option></select><small>필터는 대상을 찾는 기준이고, 이 권한이 실제 LAC ONE 멤버 권한입니다.</small></label></div><div class="setup-demo-member-summary"><div><strong>${esc(roleLabel)} 역할</strong><span>서버 전체 18명 중 ${visibleMembers.length}명 표시</span></div><button type="button" data-action="setup-demo-select-visible-members">${allVisibleSelected?'현재 목록 선택 해제':'현재 목록 전체 선택'}</button></div><div class="setup-demo-member-list">${memberRows}</div>${demo.memberImportDone?demo.memberImportSkipped?`<div class="setup-demo-member-result is-skipped"><i>→</i><div><strong>멤버 등록은 나중에 진행합니다.</strong><span>운영 중에는 Discord 우클릭 → 앱 → LAC ONE 멤버 등록으로 한 명씩 추가할 수 있습니다.</span></div></div>`:`<div class="setup-demo-member-result"><i>✓</i><div><strong>${selectedMembers.size}명 등록 체험 완료</strong><span>선택한 멤버에게 <b>${esc(targetRole)}</b> 권한을 부여하는 흐름입니다.</span></div></div>`:`<div class="setup-demo-member-actions"><span><b>${selectedVisible}명 선택됨</b> · 현재 화면은 체험 데이터입니다.</span><button type="button" data-action="setup-demo-skip-members">나중에 개별 등록</button></div>`}</div>`;
  }
  if(step===6){ const memberDone=demo.memberImportDone&&!demo.memberImportSkipped; body=`<div class="setup-demo-complete"><div class="setup-demo-complete-mark">✓</div><span class="setup-demo-kicker">SETUP COMPLETE</span><h2>운영 준비가 완료되었습니다.</h2><p>실제 초기설정에서는 여기까지 완료되면 운영 콘솔로 자연스럽게 이동합니다.</p><div class="setup-demo-next-task"><span>${memberDone?'MEMBERS':'NEXT'}</span><div><strong>${memberDone?`${selectedMembers.size}명 멤버 등록 준비 완료`:'추가 멤버는 언제든 등록할 수 있습니다.'}</strong><small>${memberDone?`${esc(targetRole)} 권한으로 일괄 등록하는 흐름을 확인했습니다.`:'Discord에서 대상 우클릭 → 앱 → LAC ONE 멤버 등록'}</small></div></div><div class="setup-demo-safe"><b>체험 모드</b><span>지금 선택한 역할·기능·채널·멤버는 실제 회사 설정에 저장되지 않았습니다.</span></div></div>`; nextLabel='체험 종료'; }
  ...
  return `<div class="setup-demo-backdrop"><section class="setup-demo-shell" role="dialog" aria-modal="true" aria-label="초기설정 가이드 체험"><aside class="setup-demo-rail"><div class="setup-demo-brand"><strong>LAC ONE</strong><small>GUIDED SETUP · PREVIEW</small></div><div class="setup-demo-preview-badge"><i></i><span>테스트 설정</span><small>실제 데이터 변경 없음</small></div><nav>${rail}</nav><button type="button" class="setup-demo-exit" data-action="close-modal">체험 닫기</button></aside><main class="setup-demo-main"><header><div><span>초기설정 가이드 체험</span><strong>${step+1} / ${steps.length}</strong></div><button type="button" data-action="close-modal" aria-label="닫기">×</button></header><div class="setup-demo-content ${scrollStep?'is-scroll-step':''}">${body}</div><footer>${step>0&&step<6?`<button type="button" class="setup-demo-back" data-action="setup-demo-back">이전</button>`:'<span></span>'}<div><button type="button" class="setup-demo-restart" data-action="setup-demo-restart">처음부터</button>${primary}</div></footer></main></section></div>`;
}
...
function setupGuideLive(state){
  ...
  if(step===2){body=`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 02</span><h2>누가 운영하고, 누가 사용하는지 정합니다.</h2><p>Discord 역할을 기준으로 LAC ONE 관리자와 일반 멤버를 구분합니다.</p><div class="setup-demo-role-grid"><label class="setup-demo-field"><span>관리자 역할</span><select data-guide-role="adminRoleId">${roleOptions(guide.adminRoleId)}</select><small>멤버 · 공금 · 자산 · 회사 설정 관리</small></label><label class="setup-demo-field"><span>일반 멤버 역할</span><select data-guide-role="memberRoleId">${roleOptions(guide.memberRoleId)}</select><small>회사 기능을 사용하는 일반 구성원</small></label></div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-save-roles" ${guide.adminRoleId&&guide.memberRoleId?'':'disabled'}>저장하고 다음</button>`;}
  ...
  if(step===4){const mode=guide.channelMode==='direct'?'direct':'quick';const modePicker=`<div class="setup-demo-channel-mode"><button type="button" class="${mode==='quick'?'is-active':''}" data-action="setup-guide-channel-mode" data-mode="quick"><b>⚡</b><span><strong>빠른 설정</strong><small>필요한 채널을 LAC ONE이 생성하고 바로 연결합니다.</small></span></button><button type="button" class="${mode==='direct'?'is-active':''}" data-action="setup-guide-channel-mode" data-mode="direct"><b>↗</b><span><strong>직접 연결</strong><small>이미 사용하는 Discord 채널을 선택합니다.</small></span></button></div>`;const permissionCard=guide.permissionIssue==='manage_channels'?`<div class="setup-guide-permission-card"><div class="setup-guide-permission-icon">!</div><div class="setup-guide-permission-copy"><strong>채널 생성 권한이 필요합니다.</strong><span>${esc(guide.permissionMessage||'LAC ONE이 채널을 만들려면 Discord의 채널 관리 권한 승인이 필요합니다.')}</span><small>현재 연결된 Discord 서버만 고정해서 권한 승인을 다시 진행합니다. 관리자 전체 권한은 요청하지 않습니다.</small></div><div class="setup-guide-permission-actions"><button type="button" class="is-primary" data-action="setup-guide-reapprove-channels">권한 다시 승인</button><button type="button" data-action="setup-guide-channel-mode" data-mode="direct">기존 채널 직접 연결</button></div></div>`:'';if(mode==='quick'){body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>채널도 LAC ONE이 준비할 수 있습니다.</h2><p>추천 이름을 그대로 쓰거나 회사 스타일에 맞게 수정한 뒤 생성합니다.</p>${modePicker}${permissionCard}${plan.length?`<div class="setup-demo-builder"><div class="setup-demo-builder-head"><div><strong>생성할 구성</strong><span>생성 전에 자유롭게 수정할 수 있습니다.</span></div><em>${plan.length}개 채널</em></div><label class="setup-demo-category-row"><span><strong>카테고리</strong><small>Discord에서 채널을 묶어둘 카테고리</small></span><input type="text" maxlength="90" value="${esc(guide.categoryName||'LAC ONE')}" data-guide-category-name></label><div class="setup-demo-create-list">${plan.map(generatedRow).join('')}</div><div class="setup-demo-builder-note"><b>LIVE</b><span>생성하면 Discord 서버에 실제 카테고리와 채널이 만들어지고 LAC ONE 기능에 자동 연결됩니다.</span></div></div>`:`<div class="setup-demo-empty"><strong>생성할 채널이 없습니다.</strong><span>선택한 기능은 별도의 Discord 채널이 필요하지 않습니다.</span></div>`}</div>`;primary=plan.length?`<button type="button" class="setup-demo-primary setup-demo-primary--generate" data-action="setup-guide-create-channels">이 구성으로 생성</button>`:`<button type="button" class="setup-demo-primary" data-action="setup-guide-next">다음</button>`;}else{body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>기존 채널을 그대로 연결할 수 있습니다.</h2><p>서버 구조를 바꾸지 않고 LAC ONE 기능만 기존 채널에 연결합니다.</p>${modePicker}<div class="setup-demo-channel-list">${directRows||`<div class="setup-demo-empty"><strong>연결할 채널이 없습니다.</strong><span>바로 다음 단계로 진행할 수 있습니다.</span></div>`}</div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-save-direct-channels">저장하고 다음</button>`;}}
  if(step===5){const filterRoleName=roles.find(r=>String(r.role_id)===String(guide.memberFilterRoleId))?.role_name||'역할';body=`<div class="setup-demo-quest setup-demo-quest--members"><span class="setup-demo-quest-no">QUEST 05</span><h2>기존 멤버를 한 번에 등록할 수 있습니다.</h2><p>Discord 역할로 회사 인원만 걸러낸 뒤 LAC ONE 권한을 일괄 부여합니다.</p><div class="setup-demo-member-toolbar"><label><span>Discord 역할로 대상 찾기</span><select data-guide-member-filter>${roleOptions(guide.memberFilterRoleId)}</select><small>게스트가 많은 서버에서도 원하는 역할의 멤버만 불러옵니다.</small></label><label><span>LAC ONE에서 부여할 권한</span><select data-guide-member-target-role><option value="member" ${guide.memberTargetRole!=='admin'?'selected':''}>일반 멤버</option><option value="admin" ${guide.memberTargetRole==='admin'?'selected':''}>관리자</option></select><small>선택한 Discord 멤버에게 부여할 LAC ONE 권한입니다.</small></label></div>${guide.memberListLoaded?`<div class="setup-demo-member-summary"><div><strong>${esc(filterRoleName)} 역할</strong><span>${Number(guide.memberScanCount||candidates.length)}명 확인 · ${candidates.length}명 표시</span></div><button type="button" data-action="setup-guide-select-members">${allSelected?'현재 목록 선택 해제':'현재 목록 전체 선택'}</button></div><div class="setup-demo-member-list">${memberRows||`<div class="setup-demo-empty"><strong>해당 역할의 등록 가능한 멤버가 없습니다.</strong><span>BOT 계정과 기존 등록 멤버는 제외될 수 있습니다.</span></div>`}</div>`:`<div class="setup-demo-empty"><strong>역할을 선택해 멤버를 불러오세요.</strong><span>Discord 서버 인원이 많아도 선택한 역할만 필터링합니다.</span></div>`}${guide.memberImportDone?guide.memberImportSkipped?`<div class="setup-demo-member-result is-skipped"><i>→</i><div><strong>멤버 등록은 나중에 진행합니다.</strong><span>Discord 우클릭 → 앱 → LAC ONE 멤버 등록으로 개별 추가할 수 있습니다.</span></div></div>`:`<div class="setup-demo-member-result"><i>✓</i><div><strong>선택한 멤버 등록을 완료했습니다.</strong><span>멤버 관리에서 등록 결과를 확인할 수 있습니다.</span></div></div>`:`<div class="setup-demo-member-actions"><span><b>${selected.size}명 선택됨</b></span><button type="button" data-action="setup-guide-skip-members">나중에 개별 등록</button></div>`}</div>`;primary=guide.memberImportDone?`<button type="button" class="setup-demo-primary" data-action="setup-guide-next">다음</button>`:guide.memberListLoaded?`<button type="button" class="setup-demo-primary setup-demo-primary--members" data-action="setup-guide-import-members" ${selected.size?'':'disabled'}>선택 멤버 등록</button>`:`<button type="button" class="setup-demo-primary" data-action="setup-guide-load-members" ${guide.memberFilterRoleId?'':'disabled'}>멤버 불러오기</button>`;}
  if(step===6){body=`<div class="setup-demo-complete"><div class="setup-demo-complete-mark">✓</div><span class="setup-demo-kicker">SETUP COMPLETE</span><h2>운영 준비가 완료되었습니다.</h2><p>설정한 역할·기능·채널이 현재 회사에 적용되었습니다.</p><div class="setup-demo-next-task"><span>NEXT</span><div><strong>이제 LAC ONE을 바로 사용할 수 있습니다.</strong><small>추가 멤버는 Discord 우클릭 → 앱 → LAC ONE 멤버 등록으로 언제든 추가할 수 있습니다.</small></div></div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-finish">운영 콘솔 시작</button>`;}
  const scrollStep=step===4||step===5;
  return `<div class="setup-demo-backdrop"><section class="setup-demo-shell" role="dialog" aria-modal="true" aria-label="초기설정 가이드"><aside class="setup-demo-rail"><div class="setup-demo-brand"><strong>LAC ONE</strong><small>GUIDED SETUP</small></div><div class="setup-demo-preview-badge is-live"><i></i><span>실제 설정</span><small>현재 회사에 단계별 적용</small></div><nav>${rail}</nav><button type="button" class="setup-demo-exit" data-action="close-modal">가이드 닫기</button></aside><main class="setup-demo-main"><header><div><span>초기설정 가이드</span><strong>${step+1} / ${steps.length}</strong></div><button type="button" data-action="close-modal" aria-label="닫기">×</button></header><div class="setup-demo-content ${scrollStep?'is-scroll-step':''}">${body}</div><footer>${step>0&&step<6?`<button type="button" class="setup-demo-back" data-action="setup-guide-back">이전</button>`:'<span></span>'}<div>${primary}</div></footer></main></section></div>`;
}
