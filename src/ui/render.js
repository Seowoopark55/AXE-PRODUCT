...
// ACCOUNTS
// ============================================================
function accountRecords(state){ const snap=state.accountsSnapshot||{}; const reqs=snap.requests||[]; const members=state.memberships||[]; return (snap.accounts||[]).filter(a=>a.member_status==='active').map(a=>{ const pending=reqs.find(r=>r.membership_id===a.membership_id&&r.status==='pending'); const member=members.find(m=>m.id===a.membership_id); return {...a,discord_display_name:a.discord_display_name||member?.discord_display_name||'',pending,status:pending?(a.account?'변경 대기':'등록 대기'):a.account&&a.enabled?'승인':'미등록'};}); }
function renderAccounts(state){
  ...
  const q=(state.accountQuery||'').toLowerCase(); if(q)rows=rows.filter(r=>`${r.display_name} ${r.discord_display_name||''} ${ROLE_KO[r.role]||r.role||''} ${r.account||''}`.toLowerCase().includes(q));
