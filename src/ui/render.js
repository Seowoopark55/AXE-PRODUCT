...
export function renderShell(root, state) {
  ...
  root.innerHTML = `
    ...
    ${!state.envReady ? renderEnvironmentMissing() : !user ? renderLogin(state) : !state.companies?.length ? renderOnboarding(state) : renderAuthed(state)}
  `;
}
    ...
    <div class="workspace-shell">
      <aside class="sidebar">
        <section class="company-switcher"><span class="overline">현재 회사</span>
          <select class="company-select runtime-company-select" data-action="switch-company" aria-label="회사 선택">${(state.companies||[]).map(c=>`<option value="${esc(c.id)}" ${c.id===state.companyId?'selected':''}>${esc(c.name)}</option>`).join('')}</select>
