...
function renderSettings(state){
  ...
  const tabs=`<nav class="ops-settings-tabs" aria-label="회사 설정 하위 메뉴"><button class="${state.settingsTab==='basic'?'is-active':''}" data-settings-tab="basic"><strong>기본 정보</strong><span>회사·Discord 공통 값</span></button><button class="${state.settingsTab==='modules'?'is-active':''}" data-settings-tab="modules" ${catalogPending?'disabled':''}><strong>기능 설정</strong><span>기능별 채널 · 사용 여부</span></button>${cookingAvailable?`<button class="${cookingTab?'is-active':''}" data-settings-tab="cooking"><strong>요리 메뉴</strong><span>회사별 주문 품목 · 가격</span></button>`:''}</nav>`;
  const action=cookingTab?`<button class="ops-settings-save-action" type="button" data-action="open-cooking-menu" ${!canAdmin(state)?'disabled':''}>${icon('plus')}<span>메뉴 추가</span></button>`:`<button class="ops-settings-save-action" form="settings-active-form" type="submit" ${resetBusy||(catalogPending&&state.settingsTab==='basic')?'disabled':''}>${icon('save')}<span>${esc(saveLabel)}</span></button>`;
...
function renderCookingMenuSettings(state){
  ...
  const admin=canAdmin(state); const active=rows.filter(x=>x.enabled!==false).length;
  return `<section class="ops-settings-board ops-cooking-menu-board"><div class="ops-settings-board-head"><div><h2>요리 주문 메뉴</h2><p>회사별 주문 품목을 관리합니다. 활성 메뉴만 Discord 주문창에 표시됩니다.</p></div><span><strong>${active}</strong> / ${rows.length} 사용 중</span></div><div class="ops-cooking-menu-note"><strong>회사별 독립 카탈로그</strong><span>메뉴 이름·설명·가격·노출 순서를 바꿔도 다른 회사에는 영향을 주지 않습니다. 수량 단위는 현재 SET 기준입니다.</span></div><div class="ops-cooking-menu-list">${rows.length?rows.map(row=>`<article class="ops-cooking-menu-row ${row.enabled===false?'is-disabled':''}"><div class="ops-cooking-menu-main"><strong>${esc(row.label||row.type_key)}</strong><span>${esc(row.short_label||row.label||row.type_key)}${row.detail?` · ${esc(row.detail)}`:''}</span></div><div class="ops-cooking-menu-price"><strong>${money(Number(row.price_per_set||0))}</strong><span>SET당 가격</span></div><div class="ops-cooking-menu-order"><strong>${Number(row.sort_order||0)}</strong><span>노출 순서</span></div><button type="button" class="runtime-power ${row.enabled===false?'is-off':'is-on'}" data-action="toggle-cooking-menu" data-type-key="${esc(row.type_key)}" ${!admin?'disabled':''}>${icon('power')}<span>${row.enabled===false?'OFF':'ON'}</span></button><button type="button" class="ops-mgmt-action" data-action="edit-cooking-menu" data-type-key="${esc(row.type_key)}" ${!admin?'disabled':''}>수정</button></article>`).join(''):`<div class="ops-cooking-menu-empty"><strong>등록된 요리 메뉴가 없습니다.</strong><span>${admin?'메뉴 추가 버튼으로 첫 주문 품목을 등록해 주세요.':'회사 관리자가 메뉴를 등록하면 여기에 표시됩니다.'}</span></div>`}</div><div class="ops-settings-module-foot"><strong>BOT 자동 반영</strong><span>메뉴 변경은 STAGING BOT의 다음 요리 snapshot 갱신부터 Discord 주문창에 반영됩니다.</span></div></section>`;
}
