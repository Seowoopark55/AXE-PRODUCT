    ...
    <div class="workspace-shell">
      <aside class="sidebar">
        <section class="company-switcher"><span class="overline">현재 회사</span>
          ...
          ${membership?.role==='owner'||state.platformAdmin?`<div class="company-quick-actions company-quick-actions--single"><button class="accent-action" data-action="open-create-company">+ 새 회사</button></div>`:''}
        </section>
...
// ============================================================
const OPS_PAGE_SIZE={questions:5,suggestions:5,fund:8,fundReview:6,members:8,assets:8,returns:8,accounts:8,cooking:9,platform:8};
...
function renderSuggestions(state){
  ...
  const header=pageHeader('PRIVATE SUPPORT','건의게시판','작성한 건의는 본인과 AXE PRODUCT 운영자만 볼 수 있습니다.',createAction);
  if(board.error){
...
function supportQuestionModal(state,m){
  ...
  const note=platform?'답변은 AXE PRODUCT 운영자만 등록할 수 있습니다. 등록하면 자동으로 답변완료가 되고 작성자에게 Discord DM 알림을 시도합니다.':'작성자는 자신의 질문에만 추가 질문을 남길 수 있습니다. 추가 질문을 보내면 상태가 다시 답변대기로 변경됩니다.';
...
function suggestionThreadModal(state,m){
  ...
  return modalShell(q.title||'건의','작성자와 AXE PRODUCT 운영자만 볼 수 있는 1:1 비공개 스레드입니다.',`<div class="support-question-detail suggestion-thread-detail"><div class="support-question-detail-head"><div><span class="suggestion-category ${cat.className}">${cat.label}</span><span class="axe-question-status ${meta.className}">${meta.label}</span>${q.unread?'<em>NEW</em>':''}</div><div><strong>${esc(q.company_name||companyDisplayName(state))}</strong><span>${esc(q.author_name||'사용자')} · ${esc(fmtDate(q.created_at))}</span></div>${statusActions}</div><div class="support-thread">${thread}</div>${replyArea}</div>`,true);
}
...
function memberRegisterModal(state){
  return modalShell('멤버 등록','팀원이 전달한 Discord ID로 현재 회사에 먼저 등록합니다.',`<form data-form="member-register" class="runtime-modal-form" autocomplete="off">
    <div class="runtime-modal-hint is-full"><strong>팀원이 첫 접속 화면에서 복사한 Discord ID를 붙여넣어 주세요.</strong><br>현재 연결된 Discord 서버의 실제 멤버인지 확인한 뒤 등록합니다. 등록 전에는 팀원이 AXE PRODUCT에 진입할 수 없습니다.</div>
...
function setupGuidePreview(state){
  ...
  const previewModules=MODULE_ORDER.map(key=>({key,ui:MODULE_UI[key]})).filter(row=>row.ui);
  const moduleCard=({key,ui})=>`<button type="button" class="setup-demo-module ${modules[key]?'is-on':''}" data-action="setup-demo-toggle-module" data-module-key="${esc(key)}"><span class="setup-demo-check">${modules[key]?'✓':''}</span><div><strong>${esc(ui.name)}</strong><small>${esc(ui.desc)}</small></div><em>${modules[key]?'ON':'OFF'}</em></button>`;
  ...
  if(step===3){ body=`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 03</span><h2>회사에서 사용할 기능만 선택합니다.</h2><p>켜지 않은 기능은 다음 단계에서도 묻지 않습니다.</p><div class="setup-demo-module-grid">${previewModules.map(moduleCard).join('')}</div></div>`; }
  if(step===4){
...
function setupGuideLive(state){
  ...
  if(step===6){body=`<div class="setup-demo-complete"><div class="setup-demo-complete-mark">✓</div><span class="setup-demo-kicker">SETUP COMPLETE</span><h2>운영 준비가 완료되었습니다.</h2><p>설정한 역할·기능·채널이 현재 회사에 적용되었습니다.</p><div class="setup-demo-next-task"><span>NEXT</span><div><strong>이제 AXE PRODUCT를 바로 사용할 수 있습니다.</strong><small>추가 멤버는 Discord 우클릭 → 앱 → AXE 멤버 등록으로 언제든 추가할 수 있습니다.</small></div></div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-finish">운영 콘솔 시작</button>`;}
  const scrollStep=step===4||step===5;
  return `<div class="setup-demo-backdrop"><section class="setup-demo-shell" role="dialog" aria-modal="true" aria-label="초기설정 가이드"><aside class="setup-demo-rail"><div class="setup-demo-brand"><strong>AXE PRODUCT</strong><small>GUIDED SETUP</small></div><div class="setup-demo-preview-badge is-live"><i></i><span>실제 설정</span><small>현재 회사에 단계별 적용</small></div><nav>${rail}</nav><button type="button" class="setup-demo-exit" data-action="close-modal">가이드 닫기</button></aside><main class="setup-demo-main"><header><div><span>초기설정 가이드</span><strong>${step+1} / ${steps.length}</strong></div><button type="button" data-action="close-modal" aria-label="닫기">×</button></header><div class="setup-demo-content ${scrollStep?'is-scroll-step':''}">${body}</div><footer>${step>0&&step<6?`<button type="button" class="setup-demo-back" data-action="setup-guide-back">이전</button>`:'<span></span>'}<div>${primary}</div></footer></main></section></div>`;
}
