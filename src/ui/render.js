    ...
    <div class="runtime-access-stage">
      <div class="runtime-access-shell runtime-access-shell--calm">
        <div class="runtime-access-panel runtime-access-panel--calm">
          <div class="runtime-access-brand-compact"><strong>LAC HUB</strong><small>LAC 회사 운영 플랫폼</small></div>
...
function firstRunContent(discord,{testMode=false,focus='',memberCheck=''}={}) {
  ...
  return `
    <header class="runtime-first-run__head"><span>LAC HUB START</span><h1>소속 회사를 확인하지 못했습니다.</h1><p>새 회사를 등록할 대표인지, 이미 LAC HUB을 이용 중인 회사의 팀원인지 선택해 주세요.</p></header>
...
function memberRegisterModal(state){
  return modalShell('멤버 등록','팀원이 전달한 Discord ID로 현재 회사에 먼저 등록합니다.',`<form data-form="member-register" class="runtime-modal-form" autocomplete="off">
    <div class="runtime-modal-hint is-full"><strong>팀원이 첫 접속 화면에서 복사한 Discord ID를 붙여넣어 주세요.</strong><br>현재 연결된 Discord 서버의 실제 멤버인지 확인한 뒤 등록합니다. 등록 전에는 팀원이 LAC HUB에 진입할 수 없습니다.</div>
    ...
    <label class="is-full">LAC HUB 권한<select name="role"><option value="member" selected>일반 멤버</option><option value="manager">매니저</option><option value="admin">관리자</option></select><small>OWNER 권한은 이 등록 화면에서 부여하지 않습니다.</small></label>
...
function setupGuidePreview(state){
  ...
  if(step===0){ body=`<div class="setup-demo-hero"><span class="setup-demo-kicker">LAC HUB SETUP GUIDE</span><h2>처음 설정도, 순서대로 하면 어렵지 않습니다.</h2><p>필요한 것만 하나씩 안내하고 완료된 단계는 자동으로 확인합니다. 지금 체험하는 내용은 실제 회사 설정에 저장되지 않습니다.</p><div class="setup-demo-facts"><span><b>약 2–3분</b> 예상 소요시간</span><span><b>6단계</b> 핵심 설정만 진행</span><span><b>0건</b> 실제 데이터 변경</span></div></div>`; nextLabel='체험 시작'; }
  ...
  if(step===4){
    ...
    if(mode==='quick'){
      ...
      body=`<div class="setup-demo-quest setup-demo-quest--channels"><span class="setup-demo-quest-no">QUEST 04</span><h2>채널도 LAC HUB이 준비할 수 있습니다.</h2><p>추천 구성을 그대로 사용하거나, 카테고리와 채널명을 회사 스타일에 맞게 바꾼 뒤 생성합니다.</p>${modePicker}${builder}</div>`;
    }else{
...
function setupGuideLive(state){
  ...
  if(step===2){body=`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 02</span><h2>누가 운영하고, 누가 사용하는지 정합니다.</h2><p>Discord 역할을 기준으로 LAC HUB 관리자와 일반 멤버를 구분합니다.</p><div class="setup-demo-role-grid"><label class="setup-demo-field"><span>관리자 역할</span><select data-guide-role="adminRoleId">${roleOptions(guide.adminRoleId)}</select><small>멤버 · 공금 · 자산 · 회사 설정 관리</small></label><label class="setup-demo-field"><span>일반 멤버 역할</span><select data-guide-role="memberRoleId">${roleOptions(guide.memberRoleId)}</select><small>회사 기능을 사용하는 일반 구성원</small></label></div></div>`;primary=`<button type="button" class="setup-demo-primary" data-action="setup-guide-save-roles" ${guide.adminRoleId&&guide.memberRoleId?'':'disabled'}>저장하고 다음</button>`;}
