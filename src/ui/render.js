...
function renderPlatform(state){
  ...
  if(q)rows=rows.filter(r=>`${r.company_name||''} ${r.owner_name||''} ${r.guild_name||''} ${r.plan||''}`.toLowerCase().includes(q));
...
function platformSubscriptionModal(state,m){
  ...
  const dateValue=v=>v?dateKey(v):''; const status=String(row.subscription_status||row.effective_status||'active');
  return modalShell('회사 구독 관리',row.company_name||'회사',`<form data-form="platform-subscription" class="runtime-modal-form"><input type="hidden" name="company_id" value="${esc(row.company_id)}"><label>플랜<select name="plan"><option value="trial" ${row.plan==='trial'?'selected':''}>TRIAL</option><option value="standard" ${row.plan==='standard'?'selected':''}>STANDARD</option><option value="pro" ${row.plan==='pro'?'selected':''}>PRO</option><option value="internal" ${row.plan==='internal'?'selected':''}>INTERNAL</option><option value="legacy" ${row.plan==='legacy'?'selected':''}>LEGACY</option></select></label><label>상태<select name="status"><option value="trial" ${status==='trial'?'selected':''}>체험</option><option value="active" ${status==='active'?'selected':''}>사용중</option><option value="paused" ${status==='paused'?'selected':''}>일시 정지</option><option value="expired" ${status==='expired'?'selected':''}>만료</option><option value="lifetime" ${status==='lifetime'?'selected':''}>무제한</option></select></label><label>시작일<input type="date" name="starts_at" value="${esc(dateValue(row.starts_at))}"></label><label>종료일<input type="date" name="ends_at" value="${esc(dateValue(row.ends_at))}"></label><label>유예 종료일<input type="date" name="grace_until" value="${esc(dateValue(row.grace_until))}"></label><label>Discord<input value="${esc(row.guild_name||'미연결')}" readonly class="is-readonly"></label><label class="is-full">관리 메모<textarea name="memo" placeholder="결제 확인, 테스트 회사 등 내부 메모">${esc(row.memo||'')}</textarea></label><div class="runtime-modal-hint is-full">이 화면은 PLATFORM OWNER만 접근할 수 있습니다. 종료일이 없거나 상태가 무제한이면 기간 제한 없이 사용합니다.</div><footer><button type="button" class="runtime-btn-ghost" data-action="close-modal">취소</button><button class="runtime-btn-primary" type="submit">구독 저장</button></footer></form>`,true);
}
...
function setupGuideLive(state){
  ...
  if(step===1){body=connected?`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 01</span><h2>Discord 서버가 연결되어 있습니다.</h2><p>연결된 서버의 역할과 채널 정보를 기준으로 다음 설정을 이어갑니다.</p><div class="setup-demo-discord-card is-connected"><i></i><div><strong>${esc(state.discordConnection?.guild_name||'Discord 서버')}</strong><span>${state.onboardingStatus?.catalog_ready===false?'역할·채널 정보를 불러오는 중':'역할·채널 동기화 완료'}</span></div><b>${state.onboardingStatus?.catalog_ready===false?'동기화 중':'연결 완료'}</b></div></div>`:`<div class="setup-demo-quest"><span class="setup-demo-quest-no">QUEST 01</span><h2>운영할 Discord 서버를 연결합니다.</h2><p>AXE BOT을 추가하고 운영에 사용할 서버를 선택합니다. 연결 후 이 가이드로 자동 복귀합니다.</p><button type="button" class="setup-demo-discord-connect" data-action="setup-guide-connect"><span>Discord</span><strong>Discord 서버 연결</strong><small>필요한 서버 권한만 요청합니다.</small></button></div>`;primary=connected?`<button type="button" class="setup-demo-primary" data-action="setup-guide-next" ${state.onboardingStatus?.catalog_ready===false?'disabled':''}>다음</button>`:`<button type="button" class="setup-demo-primary" data-action="setup-guide-connect">Discord 연결</button>`;}
