...
export function renderInfoPage(state,{standalone=false}={}){
 ...
 // Only the standalone explorer gets a compact, data-backed visual endcap.
 // Do not duplicate any catalogue records or invent item art, counts or fields.
 const listEndcap=standalone&&!searching&&rows.length>0&&rows.length<=7
  ? `<div class="game-info-list-endcap" aria-label="현재 정보 목록 요약"><span class="game-info-list-endcap__eyebrow">LAC HUB · GAME INFO</span><strong>${escapeText(filters.heading)}</strong><span>현재 분류 ${rows.length}건 · 항목을 선택해 상세 정보 확인</span></div>`
  : '';
