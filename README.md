...
- 현재 운영 화면은 뒤에 그대로 유지되어 사이트가 바뀐 듯한 혼란 방지
- 좌측 `가이드 닫기` + 우측 상단 `×`로 종료 동선 명확화
- 가이드 내부 기능 이동 시 다이얼로그를 닫고 해당 운영 화면으로 이동
- 기존 대형 가이드 타이포그래피와 기능별 튜토리얼 콘텐츠 유지

# AXE PRODUCT WEB 3.22.0

- 기본 홈: 대시보드 유지
- 상단 계정 영역을 제외한 전체 헤더 클릭 → 대시보드
- 사용 가이드를 일반 운영 화면에서 분리한 전용 Guide Center로 재설계
- 가이드 타이포그래피/카드/예시 영역 확대
