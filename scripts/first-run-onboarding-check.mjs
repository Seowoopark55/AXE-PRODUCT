...
expect('current package version',pkg.version==='1.7.41-web-ui.64');
expect('first-run explains unresolved membership instead of assuming brand-new user',render.includes('소속 회사를 확인하지 못했습니다.'));
expect('first-run exposes new company registration path',render.includes('새 회사 등록 시작')&&render.includes('data-action="open-create-company"'));
...
expect('member path can re-check registration',render.includes('data-action="check-member-registration"')&&main.includes("if(action==='check-member-registration')"));
expect('member path shows and copies Discord registration identity',render.includes('Discord ID')&&render.includes('data-action="copy-registration-info"')&&main.includes('AXE PRODUCT 멤버 등록 요청'));
