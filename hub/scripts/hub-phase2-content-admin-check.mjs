...
assert.match(render,/직접 주소 및 개별 콘텐츠 이용권/);
assert.match(api,/supabase\.rpc\('lac_list_web_content_policies'\)/);
...
assert.match(root.innerHTML,/서버 차단과 연결되지 않았으므로/);
...
console.log('HUB content settings: PASS (owner-only UI, both flags, separate authenticated reads, RPC-only writes, honest staged access warning).');
