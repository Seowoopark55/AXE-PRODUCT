...
for(const testState of [state,{...state,companies:[{id:'c1',name:'회사'}],companyId:'c1'}]){
  ...
  assert.equal((html.match(/class="hub-feature hub-feature--interactive"/g)||[]).length,4);
  assert.equal((html.match(/class="hub-feature hub-feature--pending"/g)||[]).length,0);
  ...
  assert.match(html, /<a class="hub-feature hub-feature--interactive" href="\/cook\/"\s+aria-label="LAC COOK 열기">/);
}
...
console.log('PHASE24 UPDATED PASS: same-origin BUILD and COOK links, no company gating, company/info actions preserved.');
