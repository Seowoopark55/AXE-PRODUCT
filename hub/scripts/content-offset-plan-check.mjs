...
const checks=[
  ...
  ['legacy rows display unlimited without silent DB plan conversion',render.includes('companyPlanName(plan)') && render.includes("const plan=String(row.plan||'standard')") && render.includes("legacy:'무제한'")],
  ['unlimited existing label preserved; fresh issue limited to selected term',render.includes("internal:'무제한'") && render.includes("[7,'7일']") && render.includes("[30,'30일']") && render.includes("[90,'90일']")],
