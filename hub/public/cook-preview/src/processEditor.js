...
function applyRows(rows){
  ...
  for(const entry of rows){
    if(!entry||typeof entry.process_material_name!=='string'||!Array.isArray(entry.ingredients))continue;
...
function paintOptions(){
  ...
  for(const name of processNames()){const option=el('option','',name);option.value=name;list.append(option);}
...
function fill(name){
  ...
  byId('cook-process-note').textContent=!ready?error:
    ...
    name==='빵 반죽' && !row && yields.length===1 && yields[0]==='8'
      ? '주의: 원본에는 1회 8개, 기존 AXE 호환 계산에는 1개가 적용 중입니다. 실제 게임 생산량을 확인한 뒤 저장하세요.':
...
function formPayload(){
  ...
  const ingredients=rows.map(row=>({name:row.querySelector('input[type=text]').value.trim(),
    raw:row.querySelector('input[type=number]').value}));
  if(!ingredients.length||ingredients.length>24||ingredients.some(p=>!p.name||p.name.length>90||!validInt(p.raw,999999)))
    throw Error('가공 1회 투입 재료와 수량을 확인해 주세요 (최대 24종).');
  if(ingredients.some(p=>norm(p.name)===norm(name)))throw Error('가공 결과물 자신을 투입 재료로 사용할 수 없습니다.');
  if(new Set(ingredients.map(p=>norm(p.name))).size!==ingredients.length)throw Error('중복된 투입 재료가 있습니다.');
  const known=new Set(sourceNames().map(norm));
  if(ingredients.some(p=>!known.has(norm(p.name))))throw Error('기존 재료 목록에 없는 이름이 있습니다. 이름을 확인해 주세요.');
  ...
  return {entry:{process_material_name:name,result_qty:Number(outputRaw),process_time:time,
    ingredients:ingredients.map(p=>({name:p.name,qty:Number(p.raw)})),
